...
NB_MOTS.each do  |nb_mots|
  ...
  temps_java_avec_jit = temps_moyen "java -cp . WordCount '#{fichier_mots}' 1 #{nb_mots} 0"
  temps_java_sans_jit = temps_moyen "java -Djava.compiler=NONE -cp . WordCount '#{fichier_mots}' 1 #{nb_mots} 0"
...
File.open("temps-wc.txt", 'w') { |file| file.write(result) }
