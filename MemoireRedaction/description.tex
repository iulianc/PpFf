...
%
Le principal type exporté par l'API est le type \TT{Flow}, qui permet de définir un \emph{flux de données} (Sect.~\ref{flow.sect}).
%
...
L'objectif de notre API \'etait d'obtenir une API semblable \`a celle des \TT{Stream}s de \TT{Java}~8, bas\'ee sur le cha\^inage de m\'ethodes, donc la construction d'un \TT{pipeline} de traitement dans un style qui ressemble \`a un \TT{pipeline}.

\GT{Ici, je crois qu'il faudrait dire quelques mots pour répondre au
commentaire d'un des évaluateurs: <<Bien que ce soit le cœur du
travail de maitrise, les choix de conception de l’API ne sont pas
expliqués.>>.  Comme discuté précédemment (il me semble), je ne suis
pas trop certain de ce qu'il veut dire... Je crois qu'il devrait
suffire d'expliquer, même si c'est fait un peu dans le chapitre
précédent il me semble, que l'objectif était d'obtenir une API
semblable à celle des Streams de Java 8, basée sur le chainage de
méthodes, donc la construction d'un pipeline de traitement dans un
style qui ressemble à un pipeline.}

\IC{J'ai ajouté votre idée. Je l'ai trouvé très expressive.}
...
Les m\'ethodes \TT{flatten} et \TT{flatMap} ont pour r\^ole d'aplanir un flux multiniveau. Alors que la m\'ethode \TT{flatten} cr\'ee un flux unique \`a partir du contenu des divers conteneurs, la m\'ethode \TT{flatMap} cr\'ee un flux unique \`a partir du contenu des divers conteneurs interm\'ediaires r\'esultant de l'application d'une fonction fournie en argument et appliquée \`a chaque \'el\'ement du flux. Notons que cette derni\`ere m\'ethode (\TT{flatMap}) correspond en fait à une op\'eration \TT{map} suivie immédiatement d'une opération \TT{flatten}. Les signatures pour ces deux m\'ethodes sont donn\'ees dans l'annexe~\ref{methodes-api.ann}.
...
L'{API} fournit d'autres m\'ethodes qui r\'eduisent les \'el\'ements d'un flux \`a une seule valeur. Les plus utilis\'ees sont \TT{count} et \TT{sum} En utilisant la m\'ethode \TT{count}, on peut obtenir le nombre d'\'el\'ements d'un flux. Dans le cas de la m\'ethode \TT{sum}, on peut additionner les \'el\'ements d'un flux. Une autre m\'ethode plus g\'en\'erale fournie par l'{API} de \TT{PpFf} est \TT{reduce}, qui \emph{combine} les \'el\'ements d'un flux en utilisant un \TT{Reducer}, notion pr\'esent\'ee dans la section suivante.
...
Il existe des situations o\`u la fonction \TT{accumulator} est identique avec la fonction \TT{combiner}, par exemple, celui illustr\'e dans le listing~\ref{sumElementsCollectionWithReducerParallel.c++}. L'API de \TT{PpFf} fournit une surcharge de la m\'ethode \TT{reduce} pour pouvoir omettre la fonction \TT{combiner} m\^eme si l'ex\'ecution est en parall\`ele. Le listing~\ref{sumElementsCollectionParallelWithoutCombiner.c++} montre l'exemple du listing~\ref{sumElementsCollectionWithReducerParallel.c++}, mais r\'e\'ecrit en omettant la fonction \TT{combiner}. 



\GT{Ci-bas: les évaluateurs n'ont pas semblé apprécier cette
spécification. Je suggère que tu la mettes en Annexe B plutôt que dans
le corps du mémoire --- et que tu y réfères quand même comme ci-bas,
mais sans plus.}

\IC{J'ai déplacé la spécification dans l'annexe B.}
...
\GT{Ci-bas: les évaluateurs n'ont pas semblé apprécier cette
spécification. Je suggère que tu la mettes en Annexe B (même annexe
que l'autre) plutôt que dans le corps du mémoire --- et que tu y
réfères quand même comme ci-bas, mais sans plus.}
...
\GT{Ici, je crois qu'il faudrait ajouter quelques explications pour
répondre au commentaire d'un des évaluateurs: <<La méthode reduceByKey
est-elle recommandée dans un traitement en parallèle?  Comment est
gérée la contention lors des accès aux éléments par la clé?>>.}

\IC{Ci-dessous, j'ai ajouté quelques explications.}

La m\'ethode \TT{reduceByKey} est une op\'eration complexe. Sa complexit\'e augmente encore plus lorsqu'elle est utilis\'ee en parall\`ele. La biblioth\`eque \TT{PpFf} cache cette complexit\'e. En effet, il est fortement recommand\'e de l'utiliser en parall\`ele. Le temps d'ex\'ecution diminue consid\'erablement.

Dans \TT{PpFf} chaque op\'eration dans un flux s'ex\'ecute sur un \emph{thread} diff\'erent. Une op\'eration est constitu\'ee de la fonctionnalit\'e expos\'ee par chaque m\'ethode de l'interface. Plus pr\'ecisément, la m\'ethode \TT{reduceByKey} s'ex\'ecute sur un \emph{thread} s\'epar\'ement et donc l'acc\`es \`a une cl\'e de la m\'ethode est unique par le \emph{thread} sur laquelle la m\'ethode s'ex\'ecute. Lorsque le flux est trait\'e en parall\`ele en utilisant les instances parall\`eles d'un \emph{farm}, le flux est divis\'e en sous-flux. Dans ce cas les r\'esultats partiels produits par chaque m\'ethode \TT{reduceByKey} sont ensuite combin\'es, dans le \emph{thread} principal.
...
%
La valeur de cet argument permet de sp\'ecifier le nombre d'instances parall\`eles d'un \emph{farm} entre lesquels seront r\'epartis les \'el\'ements du flux \`a traiter. Par exemple, dans le listing~\ref{wordcount.c++}, l'appel \`a la m\'ethode \TT{parallel(4)} indique de r\'epartir les \'el\'ements du flux entre quatre (4) instances parall\`eles d'un \emph{farm}.


\section{L'expressivit\'e de \TT{PpFf}}

\label{PpFfExpressivities.sect}

\'Ecrit au-dessous de la biblioth\`eque \TT{FastFlow}, \TT{PpFf} essaie de simplifier le traitement des collections de donn\'ees. Cette section montre la n\'ecessit\'e d'introduire une nouvelle biblioth\`eque. Par rapport \`a la biblioth\`eque \TT{FastFlow}, \TT{PpFf} a introduit plusieurs concepts pour faciliter le traitement des collections de donn\'ees. Dans ce qui suit, nous pr\'esenterons les plus importantes.
...
language=c++,
caption={[Un pipeline pour regrouper les \'etudiants boursiers par d\'epartement en \TT{FastFlow}.]Un pipeline pour regrouper les \'etudiants boursiers par d\'epartement en \TT{FastFlow}. Ce segment de code est un extrait d'un test unitaire. Les d\'etails exacts de l'assertion ont \'et\'e omis.},
frame=single,
float]
    // ...
    ...
    // Premiere operation dans le flux.
	...
	// Une operation de filtrage pour les etudiants boursiers.
	...
	// Une operation de regroupement des etudiants par departement.
	struct groupByDepartement : ff_node_t<Student, void> {
		CONTAINER& container;
		groupByDepartement(CONTAINER& container) :
							container(container){}
...
\TT{FastFlow} est une biblioth\`eque de base niveau qui permet de traiter des collections de donn\'ees. Travailler avec des collections n'est pas toujours facile. Par exemple, supposons que dans une universit\'e nous devons filtrer les \'etudiants boursiers ayant une moyenne d'A, puis les regrouper par d\'epartement. Le listing~\ref{GroupByDepStudentsAverageFF.c++} montre cet exemple impl\'ement\'e dans \TT{FastFlow}. On peut observer que pour chaque op\'eration dans le flux on doit impl\'ementer une nouvelle fonctionnalit\'e. De plus, il est difficile de comprendre d'un coup d'œil ce que fait le code. Les op\'erations sont d'abord d\'efinies dans diff\'erents nœuds et ensuite leurs r\'ef\'erences sont utilis\'ees dans la cr\'eation du flux. 
...
Dans \TT{PpFf} ce probl\`eme peut \^etre r\'esolu en utilisant le chainage de m\'ethodes. \TT{PpFf} propose de nombreuses m\'ethodes pr\'ed\'efinies pour les op\'erations de traitement de flux les plus utilis\'ees. Le listing~\ref{GroupByDepStudentsAverage.c++} montre l'exemple du listing~\ref{GroupByDepStudentsAverageFF.c++}, mais r\'e\'ecrit dans \TT{PpFf}. On peut observer que, le code est plus facile \`a comprendre. Les op\'erations sont d\'eclar\'ees \`a la cr\'eation du flux. De plus le nombre de lignes de code utilis\'e dans \TT{PpFf} est consid\'erablement r\'eduit comparatif \`a \TT{FastFlow}. 
...
\TT{PpFf} offre la possibilit\'e d'utiliser les expressions \emph{lambda} directement dans la cr\'eation du flux. Le listing~\ref{GroupByDepStudentsCity.c++} montre l'exemple du listing~\ref{GroupByDepStudentsCityFF.c++}, mais re\'ecrit dans \TT{PpFf}. On peut observer que dans \TT{PpFf}, il n'y a pas n\'ecessaire de cr\'eer une nouvelle op\'eration pour changer le crit\`ere de filtrage qui n'est utilis\'ee qu'une seule fois. Le code est plus clair et plus concis.

Le traitement en parall\`ele

Aujourd'hui tous les nouveaux ordinateurs de bureau et portables sont des ordinateurs multicœurs. Au lieu d'un seul processeur, ils en disposent de plusieurs. Les deux biblioth\`eques, \TT{FastFlow} et \TT{PpFf} peuvent am\'eliorer consid\'erablement le traitement d'une grande collection de donn\'ees si les ressources d'une telle machine sont explor\'ees au maximum. 
...
Par d\'efaut, chaque nœud d'un flux dans \TT{FastFlow} et \TT{PpFf} s'ex\'ecute dans un \emph{thread} diff\'erent. L'ex\'ecution d'un flux peut \^etre davantage parall\'elis\'ee s'on utilise les instances parall\`eles d'un \emph{farm}. Dans un tel cas, le flux est divis\'e en sous-flux — i.e., chaque instance d'un \emph{farm} traite un sous-ensemble des \'el\'ements. Cette approche augmente les performances, mais augment aussi la complexit\'e. Dans \TT{FastFlow} l'utilisateur doit cr\'eer explicitement chaque sous-flux. Le listing~\ref{GroupByDepStudentsAverageParallelFF.c++} montre l'exemple du listing~\ref{GroupByDepStudentsAverageFF.c++}, mais re\'ecrit en uililisant les instances parall\`eles d'un \emph{farm}. Dans le cas de \TT{PpFf}, ce m\'ecanisme est transparent pour utilisateur. Il suffit d'ajouter la m\'ethode \TT{parallel} dans le flux en sp\'ecifiant en argument le nombre de sous-flux \`a cr\'eer. Le listing~\ref{GroupByDepStudentsAverageParallel.c++} montre un exemple pour ilustrer la simplit\'e d'utilisation de \TT{PpFf}.
