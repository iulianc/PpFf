...
\centering
     \includegraphics[width=1.0\textwidth]{Figures/ComponentsAPI.jpg}
      \caption{Les composants de \ppff.}
...
\IC{L'expression m'a \'echapp\'ee à la r\'evision.}
...
\IC{Corrig\'e partout dans le document.}
...
\IC{Je l'ai corrig\'e partout dans le document.}
...
\IC{J'ai ajout\'e deux exemples pour l'op\'erateur Reducer.}
...
\IC{C'est une faute. J'ai m\'elang\'e les listings. J'ai r\'ef\'er\'e le listing 10 en place du listing 11.}
...
\IC{J'ai introduit le listing 11 pour d\'ecrire ce cas.}
...
\IC{J'ai r\'e\'ecrit ce paragraphe.}
...
\end{lstlisting}
\caption{S\'election des noms de tous les employeurs d'une collection.}
\label{mapExample2.c++}
\end{Listing}


La m\'ethode \TT{map} est utilis\'ee pour transformer une collection d'objets en un autre collection d'objets en appliquant une fonction --- typiquement une lambda-expression --- sur chacun des objets. Par exemple dans le listing~\ref{mapExample.c++} la fonction lambda-expression pass\'ee en param\`etre \`a la m\'ethode \TT{map} multiplie avec la valeur 3 chaque \'el\'ement du conteneur \TT{elems}. Dans un autre cas plus particulaire, la méthode \TT{map} permet de s\'electionner une information de chacun des objets d'une collection. Par exemple le listing~\ref{mapExample2.c++} montre un exemple o\`u la méthode \TT{map} s\'electionne les noms de tous les employeurs d'une collection. 
...
\IC{J'ai mis la description dans deux paragraphes distincts.}

Un exemple qui illustre l'utilisation de la m\'ethode \TT{find} est montr\'e dans le listing~\ref{wordcount.c++}. La m\`ethode  s\'electionne tous les \'el\'ements du flux de type \TT{string} qui ne sont pas vides.
...
L'exemple fourni dans le listing~\ref{mapExample2.c++} montre l'utilisation de la m\'ethode \TT{collect}. La m\'ethode collecte les noms de tous les employ\'es d'une collection de type \TT{Employee} dans un \TT{vector} de type \TT{string}.

\GT{Vaut mieux ne pas indenter, car parfois cela d\'epasse \`a droite.}


\IC{J'ai effac\'e les indentations.}
