...
La derni\`ere \'etape dans le traitement d'un flux est la collecte des \'el\'ements du r\'esultat. Les m\'ethodes fournies par l'API offrent quatre fonctionnalit\'es principales: 
...
L'{API} fournit d'autres m\'ethodes qui r\'eduisent les \'el\'ements d'un flux \`a une seule valeur. Les plus utilis\'ees sont \TT{count}, \TT{min} et \TT{sum}. D\'ecrites dans le tableau~\ref{methodes_api.tab}, ces m\'ethodes sont des m\'ethodes sp\'ecifiques. Autrement dit, en utilisant la m\'ethode \TT{min}, on peut seulement trouver l'\'el\'ement minimum du flux. Une autre m\'ethode plus g\'en\'erale fournie par l'{API} est \TT{reduce}. D\'ecrite dans le m\^eme tableau~\ref{methodes_api.tab}, la m\'ehode \TT{reduce} r\'eduit les \'el\'ements du flux en utilisant un \TT{Reducer}. \TT{Reducer} est illustr\'e dans la section suivante.
...
La description formelle d'un \TT{Reducer} --- fournie par mon directeur de recherche -- est pr\'esent\'ee ci-dessous.
...
La description formelle de \TT{groupByKey} --- fournie par mon directeur de recherche -- est pr\'esent\'ee ci-dessous.
