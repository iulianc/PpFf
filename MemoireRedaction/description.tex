...
\GT{Je crois qu'il faudrait ajouter une quatri\`eme cat\'egorie dans
la figure et dans le texte, car \TT{parallel} n'est pas un
collector. Peut-\^etre la cat\'egorie <<Concurrent Execution>>?}

\IC{J'ai ajout\'e la cat\'egorie dans la figure et dans le tableau.}


\begin{figure}
\centering
     \includegraphics[ height=15cm, width=9cm]{Figures/MethodsAPI.jpg}
      \caption{Les diff\'erentes m\'ethodes expos\'ees par l'{API},
     regroup\'ees selon leur fonctionnalit\'e.}
       \label{MethodesAPI.fig}
\end{figure}



Ce chapitre pr\'esente l'API de \ppff,  c'est-\`a-dire, l'interface avec laquelle interagit le d\'eveloppeur. Sa conception permet aux utilisateurs de tirer parti de la simplicit\'e d'utilisation tout en cachant la complexit\'e des m\'ecanismes concurrents utilis\'es. La figure~\ref{MethodsAPI.fig} pr\'esente une vue d'ensemble des diverses m\'ethodes export\'ees par \ppff. Sur la base du type de fonctionnalit\'e export\'ee, l'API est divis\'ee en quatre cat\'egories :  \TT{Source}, \TT{Transformation},  \TT{Collector} et \TT{ConcurrentExecution}. Et la cat\'egorie \TT{Collector} est elle-m\^eme divis\'ee deux sous-cat\'egories, selon le type de r\'esultat produit~: valeur simple (par ex., r\'esultat bool\'een ou entier) ou collection.
Le r\^ole de chaque cat\'egorie dans l'API est pr\'esent\'e dans les sections suivantes. Le chapitre pr\'esente \'egalement le code source d'un petit exemple, \TT{WordCount}, pour illustrer l'utilisation de l'API et l'effet des principales m\'ethodes.
...
\gt{Pour \^etre plus complet, mais sans trop donner de d\'etails, il
faut aussi donner la signature code de splitInWords et toLowerCase, et
indiquer ce que cela fait, sans n\'ecessairement donner tout le
d\'etail du code.}
...
\GT{L'exemple utilise Pipe?!}

\IC{Je l'ai remplac\'e pour Flux.}
...
\gt{Ce sera plus uniforme/coh\'erent de pr\'esenter toutes les \'etapes de la m\^eme fa\c{c}on, via le itemize.}


Les op\'erations d\'efinissant le pipeline sont les suivantes :
\begin{itemize}

\item  La premi\`ere op\'eration d'un pipeline sert \`a d\'efinir la source du flux. Ici, la source est
constitu\'ee par les lignes contenues dans un fichier. C'est la
m\'ethode \TT{source()} qui permet d'extraire et retourner dans
le flux les lignes du fichier. Le fichier est sp\'ecifi\'e par un nom de fichier
fourni en argument \`a la m\'ethode
\TT{source}.

\gt{Inutile de toujours repr\'eciser que c'est l'application exemple
pour le compte des mots. Le contexte appropri\'e est donn\'e plus
haut.}
...
\hline
	...
	\end{tabular} &
  	\TT{Flux\&} &
    Aplanit un flux multiniveaux en cr\'eant un flux unique \`a partir du contenu des divers conteneurs. \GT{Je ne comprends pas. Cela me semble incomplet comme explications.} \IC{J'ai modifi\'e la description de flatMap.}
    \\
\hline
	\begin{tabular}{@{}l@{}}
	\tt template<T> \\
...
La source de donn\'ees est le premier op\'erateur ajout\'e dans un flux. Sans un tel op\'erateur, \TT{PpFf} g\'en\`ere un message d'erreur en informant l'utilisateur qu'un tel op\'erateur est requis.

L'API fournit des m\'ethodes statiques pour \'emettre des donn\'ees \`a partir de diverses sources, telles que des collections ou des fichiers. Des travaux futurs pourraient \'etendre l'interface pour prendre en charge plus des sources de donn\'ees.


\GT{Ci-haut, avec les changements apport\'es \`a Pipe(), ne serait-il
pas plus juste de dire que le pipeline sera quand m\^eme ex\'ecut\'e,
mais que le flux r\'esultant sera vide et donc aucun \'el\'ement ne
sera trait\'e par les op\'erations qui suivent? Ou si tu utilise
Flow::source(...), alors il faut changer un peu les explications...}

\IC{J'ai modifi\'e un peu l'explication.}


Les signatures pour les deux m\'ethodes sont donn\'ees dans le tableau~\ref{methodes_api.tab}. Tandis que la premi\`ere m\'ethode, \TT{source(path)}, consomme les donn\'ees \`a partir d'un fichier, la deuxi\`eme m\'ethode, \TT{source(begin\_iterator, end\_iterator)}, consomme les donn\'ees \`a partir d'un conteneur STL.
...
\GT{Oops! Ci-bas, tu parles de flatten, mais cette m\'ethode n'est pas
pr\'esent\'ee dans le tableau 2.1.  Je crois qu'il faudrait l'ajouter,
par souci de compl\'etude!}

\IC{Cette m\'ethode n'existe plus. La d\'eclaration de la m\'ethode a \'et\'e chang\'ee, mais pas le texte. J'ai modifi\'e la description de la m\'ethode flatMap.}

La m\'ethode \TT{flatMap}  permet d'aplanir un flux multiniveaux en associant \`a chaque \'el\'ement du flux d'entr\'ee un conteneur de type STL, puis en cr\'eant un flux unique \`a partir du contenu des divers conteneurs. Cette op\'eration correspond en fait au chainage des op\'erateurs \TT{MapOperator} et \TT{FlatOperator}. L'\TT{API} fournit deux signatures pour la m\'ethode \TT{flatMap}. Les deux signatures sont pr\'esent\'ees dans le tableau~\ref{methodes_api.tab}. Le type du flux avant d'appliquer l'op\'erateur \TT{flatMap} est \TT{In} alors que \TT{Out} est le type du flux apr\`es le traitement de l'op\'erateur sur le flux. Le type \TT{OutContainer} est le type interm\'ediaire r\'esultant de l'application de l'op\'erateur \TT{map} sur le flux. Par exemple, dans le listing~\ref{wordcount.c++}, \TT{OutContainer} est de type \TT{Words} --- un vecteur de chaines de caract\`eres. Dans cet exemple, les lignes d'un fichier divis\'ees en mots sont accumul\'ees dans un conteneur et ensuite le contenu du conteneur est transmis sur le flux.


\subsection{Find}
Une op\'eration courante consiste \`a d\'eterminer si certains \'el\'ements d'un ensemble de donn\'ees satisfont une certaaine propri\'et\'e. L'API de \TT{PpFf} fournit une telle fonctionnalit\'e via la m\'ethode \TT{find}. D\'ecrite dans le tableau~\ref{methodes_api.tab}, la m\'ethode \TT{find} s\'electionne les \'el\'ements d'un flux selon un pr\'edicat de sorte que seuls les \'el\'ements qui  satisfont le pr\'edicat sont envoy\'es \`a l'\'etape suivante. \`A noter qu'il est obligatoire que le pr\'edicat retourne une expression bool\'eenne. 

Un exemple qui illustre l'utilisation de la m\'ethode \TT{find} est pr\'esent\'e dans le listing~\ref{wordcount.c++}~: l'appel \`a la m\'ethode \TT{find}  s\'electionne tous les \'el\'ements du flux de type \TT{string} qui ne sont pas vides --- via un appel \`a la fonction \TT{notEmpty}.
...
Afin de collecter les \'el\'ements d'un flux dans un conteneur, l'{API} fournit la m\'ethode \TT{collect}, d\'ecrite dans le tableau~\ref{methodes_api.tab}. Cette m\'ethode retourne un conteneur {STL}. Le type pour les \'el\'ements du conteneur et le type du conteneur sont donn\'es par les types indiqu\'es en param\`etres \TT{template} de la m\'ethode \TT{collect}. 
...
\gt{Je ne trouve pas tellement bien d'avoir une section 2.5.2.1 et pas
d'autres sous-sections aussi profondes.  Donc, vaut mieux mettre au
m\^eme niveau que les autres sous-sections.}
...
Un op\'erateur de classe \TT{Reducer} est utilis\'e pour sp\'ecifier comment r\'eduire les \'el\'ements d'un flux \`a une valeur unique. Le listing~\ref{reducer.c++} présente la spécification  de la classe \TT{Reducer} avec les signatures des fonctions associées. 
...
\GT{Est-ce vraiment le cas ci-bas que c'est <<la valeur par d\'efaut
du type de cette valeur>>? Quelle est la valeur par d\'efaut pour
string: chaine vide? Quelle est la valeur par d\'efaut pour une classe
arbitraire? Pas certain...}

\IC{J'ai enlev\'e la partie concernant l'initialisation.}


Un \TT{Reducer} est construit avec une valeur initiale, une fonction \TT{accumulator} et une fonction \TT{combiner}. La valeur initiale et la fonction \TT{combiner} sont optionnelles, et ce gr\^ace \`a la surcharge du constructeur. Lorsque la fonction \TT{combiner} est omise, elle est remplac\'ee par la fonction \TT{accumulator}. Le listing~\ref{sumElementsCollectionWithReducer} montre un exemple d'utilisation de l'op\'erateur \TT{Reducer}. La valeur initiale est sp\'ecifi\'ee comme \'etant \TT0 et la fonction \TT{combiner} est l'addition enti\`ere \TT{std::plus<int>\{\}}. 
...
\caption{Sp\'ecification du type \TT{Reducer}.}
...
\caption{Sp\'ecification de la m\'ethode \TT{groupByKey}.}
