...
D\'enotons comme suit un \emph{map} $M$ qui associe la cl\'e $k_i$ \`a
la valeur $v_i$, pour $i=0, \ldots, n$~:%
%
\footnote{Dans du code C++, un tel \emph{map} serait repr\'esent\'e
comme suit~: $\{\{k_0, v_0\}, \ldots, \{k_n, v_n\}\}$}
%
\begin{itemize}
\item $M = \{ k_i \mapsto v_i~\vert~0 \leq i \leq n \}$
\end{itemize}
...
%
\begin{itemize}
\item $S.\TT{groupByKey}(fk, fv) = \{ k \mapsto vals(k, S)~\vert~k \in
cles(S)\}$

\item[] O\`u
\begin{itemize}
\item $cles(S) = \{ fk(s_i)~\vert~s_i \in S \}$
\item $vals(k, S) = \{ fv(s_i)~\vert~ s_i\in S \wedge fk(s_i) = k\}$
\end{itemize}
