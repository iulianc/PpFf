...
\GT{Je crois qu'il faut revoir le paragraphe qui suit, que j'ai essayé
de modifier un peu mais qui ne me semble encore pas assez clair~:( Je
crois que tu dois plutôt traiter du fait que chaque instance crée et
manipule son propre container, son propre map, donc sans interférence
avec les threads qui traitent les autres sous-flux. Puis, lors de la
combinaison des résultats des sous-flux, ces divers containers/maps
sont fusionnés les uns avec les autres dans le thread principal. Ou
quelque chose du genre... selon ce que j'ai compris de ta mise en
oeuvre.}

\IC{J'ai ajouté votre idée. C'est plus clair.}

Lorsque le flux est trait\'e en parall\`ele, il est divis\`e en sous-flux. Chaque instance cr\'ee et manipule son propre container, son propre \emph{map}, donc sans interf\'erence avec les \emph{threads} qui traitent les autres sous-flux. Puis, lors de la combinaison des r\'esultats des sous-flux, ces divers \emph{containers/maps} sont fusionn\'es les uns avec les autres dans le \emph{thread} principal.
...
\GT{Ci-bas: cela arrive souvent (il me semble), que tu écrives deux
fois des suites de phrase qui disent presque la même chose, donc ça
fait redondant~:( J'ai essayé de simplifier et d'oter la redondance.}

\IC{Je n'ai pas observé cela jusqu'à ce que vous ayez fait cette remarque.}


\'Ecrit au-dessous de la biblioth\`eque \TT{FastFlow}, \TT{PpFf} essaie de simplifier le traitement des collections de donn\'ees par rapport à \TT{FastFlow}. Dans ce qui suit, à l'aide d'exemples simples, nous pr\'esenterons ce qui distingue \ppff\ de \TT{FastFlow}.



\GT{Pour séparer en sous-parties, il vaut quand même mieux introduire
des sous-sections, quitte à ne pas les numéroter pour ne pas alourdir
la table des matières et ne pas avoir trop de niveaux numérotés.}

\IC{C'est simple et efficace.}


\subsection*{Chainage de m\'ethodes et expressions \emph{lambda}}

\GT{J'ai regroupé les deux sous-sections en une seule car ces deux
aspects sont intimement liés, le chainage de méthode ayant nettement
moins de pertinence ou d'intérêt si les expressions lambdas ne sont
pas disponibles.  De plus, introduire les détails des listings 2.14 et
2.15 ne me semblait pas utile car cela allongeait inutilement cette
section, puisque ces deux exemples étaient à toute fin pratique
identiques au premier, sauf pour une seule ligne (condition de
sélection).}

\IC{J'ai séparé le listing contenant l'exemple de FastFlow dans 2 listings pour qu'il entre dans une page.}

\begin{lstlisting}[
label={FastFlowOperationsToGroupStudentsByDep.c++},
language=c++,
caption={[Les op\'erations d'un flux \TT{FastFlow} pour regrouper les \'etudiants boursiers par d\'epartement.]Les op\'erations d'un flux \TT{FastFlow} pour regrouper les \'etudiants boursiers par d\'epartement.},
frame=single,
float]
...
language=c++,
caption={[Un pipeline \TT{FastFlow} pour regrouper les \'etudiants boursiers par d\'epartement.]Un pipeline \TT{FastFlow} pour regrouper les \'etudiants boursiers par d\'epartement. Les op\'erations sont d\'efinies dans le listing~\ref{FastFlowOperationsToGroupStudentsByDep.c++}. Ce segment de code est un extrait d'un test unitaire. Les d\'etails exacts de l'assertion ont \'et\'e omis.},
frame=single,
float]
...
Les listings~\ref{FastFlowOperationsToGroupStudentsByDep.c++} et \ref{GroupByDepStudentsAverageFF.c++} montrent un exemple pour ce problème impl\'ement\'e avec \TT{FastFlow}. Le Listing~\ref{FastFlowOperationsToGroupStudentsByDep.c++} contient les définitions des opérations requises pour créer le flux. La création du flux est illustrée dans le listing~\ref{GroupByDepStudentsAverageFF.c++}. On peut observer que pour chaque op\'eration dans le flux, on doit définir une nouvelle fonction qui traite chaque élément. De plus, il est difficile de comprendre d'un coup d'œil ce que fait le code: les op\'erations sont d'abord d\'efinies dans diff\'erents nœuds (\TT{struct} de type \TT{ff\_node} ou \TT{ff\_node\_t}) et sont ensuite utilis\'ees dans la cr\'eation du flux. 
...
\GT{(Suite à ma remarque précédente et aussi...) Je suis pas mal
certain qu'un des évaluateurs va trouver que ce qui était au début de
ce paragraphe, que j'ai supprimé, n'aurait pas été assez bien
expliqué/justifié --- trop général (coût/difficulté de traiter de
nouvelles exigences).}

\IC{Votre remarque m'a fait rire.}
...
%
Il suffirait donc de modifier l'appel à \TT{find} comme suit lors de la création du pipeline pour sélectionner les étudiants provenant de Montréal~: 
{
\begin{lstlisting}[language=c++]
		.find<Student>( 
