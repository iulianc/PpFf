...
Dans cette section, nous examinons, de façon \emph{informelle}, l'expressivit\'e de \TT{PpFf} en la comparant \`a celles de \TT{FastFlow} et \TT{Java} à l'aide d'exemples.
...
Par exemple, supposons que dans une universit\'e, on doit identifier les \'etudiants boursiers ayant une moyenne de \TT{A}, puis les regrouper par d\'epartement --- pour simplifier l'exemple, on suppose que les seules notes littérales permises sont \TT{A}, \TT{B}, etc., donc sans \TT{+}/\TT{-}
...
language=c++,
caption={[Un pipeline \ppff\ pour regrouper les \'etudiants boursiers par d\'epartement.]Un pipeline \ppff\ pour regrouper les \'etudiants boursiers par d\'epartement. Le type \TT{auto} a été utilisé pour les paramètres des expressions lambda, pour rendre le code plus succinct.\ExtraitTestUnitaire},
frame=single,
float]
...
\subsection*{Regroupement des étudiants boursiers par département}

L'interface fournie par \TT{PpFf} s'inspire de celle introduite pour les \TT{Streams} de \TT{Java} 8. Bas\'e sur le cha\^inage de m\'ethodes, un programme utilisant la bibliothèque \TT{PpFf} est assez semblable \`a un programme utilisant des \TT{Streams}. Dans ce qui suit, \`a l'aide de deux exemples, nous pr\'esentons quelques similitudes et différences entre \TT{PpFf} et \TT{Java}.

\begin{lstlisting}[
label={GroupByDepStudentsAverageParallel.java},
language=java,
caption={Un pipeline \TT{Java} pour regrouper en parall\`ele les \'etudiants boursiers par d\'epartement},
frame=single,
float]
...
Le premier exemple est le m\^eme que celui pr\'esent\'e précédemment, où on veut regrouper, par d\'epartement, les \'etudiants boursiers ayant une moyenne de \TT{A}. Les codes pour les deux versions \'equivalentes sont pr\'esent\'es dans les listings~\ref{GroupByDepStudentsAverageParallel.c++} (\TT{PpFf}) et~\ref{GroupByDepStudentsAverageParallel.java} (\TT{Java}). 

\goodbreak
\begin{minipage}{\linewidth}
La premi\`ere m\'ethode utilis\'ee dans les deux programmes permet d'extraire les donn\'ees de la source, source qui est constitu\'ee d'une collection d'objets du type \TT{Student}:
\begin{lstlisting}[language=c++]
	// PpFf 
	...
	students.stream()
\end{lstlisting}
Dans \TT{PpFf}, cette m\'ethode est une m\'ethode statique de la classe \TT{Flow}. Par contre, en \TT{Java}, on cr\'ee le flux via la m\'ethode d'instance \TT{stream()} de la classe \TT{Collection}.

\end{minipage}

\GT{Ci-haut~: J'ai supprime la dernière phrase, car on pourrait
facilement avoir la meme chose en Java avec
students.subList(...).stream();}

\IC{Ma faute. Je n'ai pas vérifié que Java pouvait soustraire une partie d'une collection.}

\GT{Remarque générale: Ne cherche pas trop à trouver des <<avantages>>
à PpFf, car les évaluateurs seront aussi très bons trouver des
<<points faibles>> s'il y en a. Il vaut mieux être <<honnête>> et
direct quand aux désavantages de PpFf. Exemple~: voir l'ajout que j'ai
fait concernant find vs. filter!}


\GT{Je ne crois pas que tu doives donner chaque instruction cote à
cote.  Tu peux simplement écrire une phrase qui explique, sans prendre
4 lignes (+ lignes blanches) de code qui répète ce qu'on trouve dans
le listing, sauf si c'est important de bien distinguer.}


La deuxi\`eme m\'ethode, qui a le m\^eme nom pour les deux versions, est la m\'ethode \TT{parallel}.
%
Comme expliqué précédemment, la parall\'elisation se fait de façon diff\'erente dans les deux programmes. \TT{PpFf} ex\'ecute chaque op\'eration d'un pipeline sur un \emph{thread} diff\'erent, donc avec du parallélisme de flux. Lorsque la m\'ethode \TT{parallel} est encha\^in\'ee, le traitement du flux est davantage parall\'elis\'e, en utilisant en plus du parallélisme de données. La constante pass\'ee en argument \`a \TT{parallel()} permet de sp\'ecifier le nombre d'instances d'un \TT{farm} à utiliser. Par contre, \TT{Java} g\`ere la cr\'eation de \emph{threads} par l'interm\'ediaire du \emph{framework} \TT{ork/join} (cf.~section~\ref{forkjoin.sect}).
%
Nous verrons, à la section~\ref{limitesppff.sect}, sous quelles conditions l'approche de
\ppff\ est préférable\ldots\ ou pas.


\GT{Ci-bas: il faut éviter de répéter des choses que tu as déjà dites
ailleurs... sinon les évaluateurs vont trouver cela redondant.}

\GT{Ci-haut et ci-bas: j'ai remplacé les samepage par des
minipage. J'avais déjà constaté que les samepage ne fonctionnaient pas
toujours, mais ce n'est ce matin --- malgré que j'écris du \LaTeX\
depuis plus de 25 ans -- que j'ai enfin compris qu'on pouvait utiliser
minipage à la place, pour être certain que ça ne brise pas en plein
milieu! Jamais trop tard pour apprendre~;)} 

\IC{Vous avez ajouté des instructions LaTex utiles qui sont difficiles à trouver sur internet et vous continuez à en ajouter des nouvelles.}

\begin{minipage}{\linewidth}
Comme \TT{Java}, \TT{PpFf} permet l'utilisation d'expressions \emph{lambdas}, qui rendent le code plus concis. Dans notre exemple, les expressions \emph{lambda} sont utilis\'ees pour filtrer les \'etudiants ayant une moyenne de \TT{A}~:
\begin{lstlisting}[language=c++]
	// PpFf 
...
\end{lstlisting}
Les deux m\'ethodes, \TT{find} de \TT{PpFf} et \TT{filter} de \TT{Java}, ont les m\^emes fonctionnalit\'es.
%
Par contre, en \ppff, à cause de la façon dont nous avons mis en
\oe{}uvre la bibliothèque avec des \emph{templates} génériques,
certaines informations additionnelles de type doivent être fournies
--- <<\TT{find<Student>}~>> vs.\ simplement <<\TT{filter}>>.
\end{minipage}

\goodbreak
\begin{minipage}{\linewidth}
Tant \TT{PpFf} que \TT{Java} adopte un m\'ecanisme d'\'evaluation paresseuse, qui retarde l'\'etape du calcul jusqu'\`a ce que le r\'esultat soit n\'ecessaire. Dans les deux cas, le traitement sur les \'el\'ements du flux ne démarre qu'apr\`es l'encha\^inement d'une m\'ethode finale de collecte (agrégation) des éléments:
\begin{lstlisting}[language=c++]
	// PpFf 
	.groupByKey<Student, Dept, Student>( 
		[](auto s) { return &(s->department); } );
	...
	.collect( Collectors.groupingBy( s -> s.department ) );
\end{lstlisting}
Alors que \ppff\ possède une méthode explicite pour regrouper les
éléments par clé, \TT{Java} définit plutôt, par l'intermédiaire de la
classe \TT{Collectors}. diverses méthodes de regroupement --- plus
d'une quarantaine. L'approche \ppff\ semble plus simple. Par contre,
là aussi des informations additionnelles de type doivent être fournies
pour les \emph{templates}.
\end{minipage}

\subsection*{\TT{WordCount}}


\GT{Ici, pas besoin d'aller autant dans les détails que pour l'exemple
précédent. Présente la version Java et souligne \emph{quelques points
nouveaux}.}

\GT{J'ai mis les listings, parce que je voulais m'assurer que les deux
<<fittaient>> sur la même page. C'est le cas uniquement si on met dans
un listing à part la classe ReduceByKey.  Il faut donc expliquer tout
d'abord cette classe, en expliquant qu'elle a été définie pour faire
en sorte que les deux versions soient aussi semblables que
possible. Ensuite, tu peux présenter les deux définitions de
Stream/Flow.}

\IC{J'ai comparé les deux programmes en essayant de suivre vos conseils.}


\begin{minipage}{\linewidth}
\begin{lstlisting}[
label={reduce-by-key.java},
language=java,
caption={La classe \TT{ReduceByKey} utilisée par l'application \TT{WordCount} en Java.},
frame=single]
...
language=c++,
gobble=4,
caption={L'objet \TT{Reducer} utilisé par l'application \TT{WordCount} en \ppff.},
frame=single]
    Reducer<std::string, int> reducer(
             0, 
             [](int count, std::string _) { return count + 1; },
...
\begin{lstlisting}[
label={wordcount-bis.c++},
language=c++,
gobble=4,
caption={Des extraits du code source de l'application \TT{WordCount} en \ppff.},
frame=single]
    typedef std::vector<std::string> Words;
...
\begin{lstlisting}[
label={wordcount.java},
gobble=8,
language=java,
caption={Des extraits du code source de l'application \TT{WordCount} en Java.},
frame=single]
...
Le deuxi\`eme exemple pr\'esent\'e est l'application \TT{WordCount}, la m\^eme application qui a servi de r\'ef\'erence pour nos exp\'eriences. Ce programme compte le nombre d'occurrences des divers mots dans un fichier texte. Les codes pour les deux versions \'equivalentes sont pr\'esent\'es dans les listings~\ref{wordcount-bis.c++} (\TT{PpFf}) et~\ref{wordcount.java} (\TT{Java}).

La premi\`ere m\'ethode utilis\'ee permet d'extraire les lignes \`a partir d'un fichier. Le nom du fichier est pass\'e en param\`etre aux m\'ethodes statiques \TT{source} dans le cas de \TT{PpFf} et \TT{Files.lines} dans le cas de \TT{Java}.

Pr\'esent\'ee dans l'exemple pr\'ec\`edent, la deuxi\`eme m\'ethode encha\^in\'ee, la m\'ethode \TT{parallel}, permet de parall\'eliser le traitement de donn\'ees.
 
Les deux suivantes m\'ethodes utilis\'ees dans notre exemple, \TT{flatMap} est \TT{map} sont des m\'ethodes avec le m\^eme nom pour les deux programmes. Tant pour \TT{PpFf} que pour \TT{Java}, les m\'ethodes prennent comme argument une fonction qui s'applique sur chaque \'el\'ement du flux. La m\'ethode \TT{flatMap} d\'ecompose chaque ligne en mots individuels et la m\'ethode \TT{map} transforme les lettres majuscules du mot en minuscules.

La derni\`ere m\'ethode encha\^in\'ee regroupe les mots similaires ensemble et compte le nombre d'occurrences de chaque mot. Alors que \TT{PpFf} r\'ealise ces op\'erations \`a l'aide de l'objet \TT{reducer} (le listing~\ref{reducer.c++}) pass\'e en argument \`a la m\'ethode \TT{reduceByKey}, \TT{Java} utilise la classe \TT{ReduceByKey}. Illustr\'ee dans le listing~\ref{reduce-by-key.java}, cette classe a \'et\'e d\'efinie pour faire en sorte que les deux versions soient aussi semblables que possibles.
