...
\begin{figure}
\centering
     \includegraphics[scale=1.5]{Figures/flow-details.pdf}
...
\section{Exemple~: l'application {WordCount}}
...
\section{Flux de donn\'ees: type {Flow}}
\label{flow.sect}
...
\section{Cat\'egorie {Source} : m\'ethodes \TT{source}}
...
\section{Cat\'egorie {Transformation}}
...
\section{Expressivit\'e de {PpFf} par rapport à {FastFlow} et
{Java}}

\label{PpFfExpressivities.sect}

Dans cette section, nous examinons, de façon \emph{informelle}, l'expressivit\'e de \TT{PpFf} en la comparant \`a celles de \TT{FastFlow} et \TT{Java} à l'aide de quelques exemples.


\subsection{{PpFf} vs.\ {FastFlow}}
...
Le deuxi\`eme exemple que nous pr\'esentons est pour l'application \TT{WordCount}, application qui a servi aussi dans nos exp\'eriences (cf.~Sect.~\ref{wordcount.sect}). Ce programme compte le nombre d'occurrences des divers mots dans un fichier texte. Les codes pour les deux versions \'equivalentes sont pr\'esent\'es dans les listings~\ref{wordcount-bis.c++} (\TT{PpFf}) et~\ref{wordcount.java} (\TT{Java}), lesquels utilisent des éléments présentés dans les listings~\ref{reducer-wordcount.c++} (\ppff), et~\ref{reduce-by-key.java} (\TT{Java}).

\begin{itemize}
\item La premi\`ere m\'ethode utilis\'ee permet d'extraire les lignes \`a partir d'un fichier. Le nom du fichier est pass\'e en param\`etre aux m\'ethodes statiques \TT{source} dans le cas de \TT{PpFf} et \TT{Files.lines} dans le cas de \TT{Java}.

\item Pr\'esent\'ee dans l'exemple pr\'ec\`edent, la deuxi\`eme m\'ethode, \TT{parallel}, permet dans les deux cas de parall\'eliser le traitement de donn\'ees.
 
\item  Les deux m\'ethodes suivantes, \TT{flatMap} est \TT{map}, ont les m\^emes noms. Tant pour \TT{PpFf} que pour \TT{Java}, les m\'ethodes prennent comme argument une fonction qui s'applique sur chaque \'el\'ement du flux. La m\'ethode \TT{flatMap} avec son argument d\'ecompose chaque ligne en mots individuels alors que la m\'ethode \TT{map} avec son argument transforme les lettres majuscules en minuscules.

\item La derni\`ere m\'ethode regroupe les mots similaires ensemble et compte le nombre d'occurrences de chaque mot. Alors que \TT{PpFf} r\'ealise ces op\'erations \`a l'aide d'un objet \TT{Reducer} (cf.~listing~\ref{reducer-wordcount.c++}) pass\'e en argument \`a la m\'ethode \TT{reduceByKey}, notre version \TT{Java} utilise un objet de classe \TT{ReduceByKey} (cf.~listing~\ref{reduce-by-key.java}), classe d\'efinie pour faire en sorte que les deux versions soient aussi semblables que possible.

Ici, la classe \TT{ReduceByKey} met en \oe{}uvre l'interface
\TT{Consumer<String>}, donc définit une méthode \TT{accept} ---
équivalente à la méthode \TT{accumulator} de \TT{Reducer} --- utilisée
comme deuxième argument de \TT{collect}.  Elle définit aussi une
méthode \TT{combine} --- équivalente à la méthode \TT{combiner} de
\TT{Reducer} --- utilisée comme troisième argumentde \TT{collect},
pour combiner les résultats intermédiaires.
%
Finalement, pour obtenir le \TT{Map} encapsulé par l'objet \TT{ReduceByKey}, un \emph{getter} simple est aussi défini.

\end{itemize}

Comme on le constate, les deux versions sont très semblables. Les
principales différences sont qu'en \ppff, il faut spécifier certains
types additionnels pour les \emph{templates} génériques et spécifier
de façon explicite le niveau de parallélisation désiré. Par contre,
l'appel à la méthode \TT{reduceByKey} avec un \TT{Reducer} semble plus
simple que la spécification d'une nouvelle classe.
