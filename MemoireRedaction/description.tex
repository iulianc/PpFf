...
La m\'ethode \TT{map} est utilis\'ee pour transformer une collection d'objets en un autre collection d'objets en appliquant une fonction --- typiquement une expression lambda --- sur chacun des objets. Par exemple, dans le listing~\ref{mapExample.c++}, l'expression lambda pass\'ee en param\`etre \`a la m\'ethode \TT{map} multiplie par 3 chaque \'el\'ement re\c{c}u. 
%
\GT{Je crois qu'il faudrait ajouter quelques explications sur le fait que ce sont toujours des pointeurs qui sont transmis, et expliquer bri\`evement pourquoi, i.e., contrainte li\'ee \`a la mise en \oe{}uvre avec Fastflow, non?}
%
