...
\centering
     \includegraphics[width=1.0\textwidth]{Figures/Pipeline.jpg}
      \caption[Repr\'esentation graphique d'un pipeline.]{Repr\'esentation graphique d'un \TT{pipeline}. Le \TT{pipeline} compos\'e de plusieurs op\'erateurs peut \^etre dispos\'e en parall\`ele ou en s\'erie. \GT{Donner des explications additionnelles~: voir plus bas.} \IC{J'ai ajout\'e un peu plus des explications ici et plus bas.}}
       \label{Pipeline.fig}
\end{figure}
...
pourrait \^etre facilement r\'ealis\'e en ayant plusieurs \TT{Pipeline}s identiques connect\'es \`a la m\^eme entr\'ee et sortie respectivement. La figure~\ref{Pipeline.fig} illustre ce m\'ecanisme. Le parall\'elisme de donn\'ee est r\'ealis\'e en connectant plusieurs pipelines en parall\`ele. Dans cette figure l'entr\'ee pour les pipelines parall\`ele est la sortie du pipeline s\'equentiel et la sortie des pipelines parall\`ele est l'entr\'ee dèun autre pipeline s\'equentiel.
...
Le listing~\ref{groupByKeyExample.c++} montre un exemple o\`u les employ\'es sont regroup\'es selon leur \^age. Plus pr\'ecis\'ement, ce listing pr\'esente un des tests unitaires pour \TT{groupByKey}, qui montre que les employ\'es d'un conteneur sont regroup\'es dans un map ou la cl\'e est l'\^age d'employeurs et la valeur du map est repr\'esent\'ee par un conteneur contenant les employeurs de même cat\'egorie d'\^age.
%
...
\IC{J'ai ajout\'e plus des explications pour cet exemple et j'ai ajout\'e aussi un autre exemple, cette fois en utilisant le deuxi\`eme param\`etre de la m\'ethode.}
...
La m\'ethode \TT{groupByKey} est d\'efinie avec deux param\`etres. Le deuxi\`eme param\`etre de la m\'ethode est optionnel. Il est repr\'esent\'e par une fonction qui s'applique sur la valeur du \TT{map} de sortie. Par exemple, en prenant le m\^eme exemple d\'efini dans le listing~\ref{groupByKeyExample.c++}, cette fois nous regroupons les noms d'employeurs selon leur \^age. Le listing~\ref{groupByKeyExample2.c++} pr\'esente un tel exemple ou les noms d'employ\'es sont regroup\'es dans un \TT{map}. La cl\'e du \TT{map} est l'\^age d'employeurs et la valeur du map est repr\'esent\'ee par un conteneur contenant les noms d'employeurs de m\^eme cat\'egorie d'\^age.
...
escapechar=\#,
language=c++,
caption={[Un pipeline pour regrouper et compter les employ\'es selon leur \^age.]Un pipeline pour regrouper et compter les employ\'es selon leur \^age. Ce segment de code est un extrait d'un test unitaire. Les d\'etails exacts de l'assertion ont \'et\'e omis.},
frame=single,
float]
...
\IC{J'ai ajout\'e un exemple de tests unitaire.}
