...
\GT{Ci-bas (\`a rev\'erifier partout dans ce chapitre!!). J'ai
supprim\'e les r\'ef\'erences \`a la notion de <<Op\'erateur>> ---
i.e., la classe associ\'ee \`a une m\'ethode --- sauf pour Reducer.
Cet aspect me semble un d\'etail li\'e \`a la mise en oeuvre. Dans la
description de l'API, il suffit de parler des m\'ethodes
elles-m\^emes!}

\IC{Il semble correct.}
...
\GT{Dans la figure: linesFromFile apparait encore. Est-elle encore
pr\'esente dans l'API? Car elle n'apparait plus dans le tableau 2.1!}

\IC{Elle n'est plus pr\'esente dans l'API. J'ai oubli\'e de modifier la figure.}

\GT{Apr\`es relecture, je trouve que les noms des cat\'egories ne sont
pas tout \`a fait justes et uniformes --- parfois un substantif passif
(transformation), parfois actif (collector). 
%
En outre, j'avais propos\'e (je crois) ConcurrentExecution mais ce
serait plus ParallelExecution, car on veut vraiment obtenir du
parall\'elisme, pas juste de la concurrence!
%
Apr\`es r\'eflexion, je propose les cat\'egories suivantes~: Source,
Transformation, Aggregation, Execution. J'ai fait diverses
modifications en ce sens... mais il faut aussi modifier la figure!}

\IC{J'ai modifi\'e la figure.}
...
Les signatures pour les deux m\'ethodes sont donn\'ees dans le tableau~\ref{methodes_api.tab} (p.~\pageref{source.page}). Tandis que la premi\`ere m\'ethode, \TT{source(path)}, produit les donn\'ees d'un flux \`a partir de contenu (les lignes) d'un fichier texte, la deuxi\`eme m\'ethode, \TT{source(begin\_iterator, end\_iterator)}, produit les donn\'ees d'un flux \`a partir des \'el\'ements d'un conteneur STL.


\begin{lstlisting}[
label={mapExample.c++},
language=c++,
caption={La transformation d'une collection d'entiers en un autre collection d'entiers en appliquant une expression lambda sur chacun des \'el\'ements.},
frame=single,
float]
...
label={mapExample2.c++},
language=c++,
caption={La s\'election des noms de tous les employ\'es d'une collection.},
frame=single,
float]
...
La m\'ethode \TT{map} est utilis\'ee pour transformer une collection d'objets en un autre collection d'objets en appliquant une fonction --- typiquement une expression lambda --- sur chacun des objets. Par exemple, dans le listing~\ref{mapExample.c++}, l'expression lambda pass\'ee en param\`etre \`a la m\'ethode \TT{map} multiplie par 3 chaque \'el\'ement re\c{c}u. Une autre utilisation typique de la méthode \TT{map} consiste \`a s\'electionner une information de chacun des objets d'une collection. Par exemple, le listing~\ref{mapExample2.c++} montre un exemple o\`u la méthode \TT{map} permet d'obtenir les noms des divers employ\'es d'une collection. 


\subsection{M\'ethode \TT{flatMap}}
...
\GT{Il faut aussi dire quelques mots sur cette cat\'egorie, m\^eme si
elle ne contient qu'une seule m\'ethode --- par souci de
compl\'etude!}

\IC{J'ai ajout\'e quelques mots.}

La m\'ethode \TT{parallel} d\'efinie dans cette cat\'egorie d\'etermine le comportement d'ex\'ecution du flux. Lorsqu'elle est ajout\'ee dans le flux, toutes les op\'erations suivant cette m\'ethode seront ex\'ecut\'ees en parall\`ele. La signature pour la m\'ethode \TT{parallel} est donn\'ee dans le tableau~\ref{methodes_api.tab}. La valeur de type enti\`ere fournie en argument permet de r\'epartir les \'el\'ements du flux entre divers \emph{threads}. Par exemple, dans le listing~\ref{wordcount.c++}, la méthode \TT{parallel(4)} permet de r\'epartir les \'el\'ements du flux entre quatre \emph{threads} et donc d'ex\'ecuter les \'etapes qui suivent en parall\`ele.
