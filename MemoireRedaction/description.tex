...
\begin{itemize}
	...
	\item Regroupement d'\'el\'ements;
	
	\item Regroupement et r\'eduction d'\'el\'ements;
\end{itemize}

\subsubsection{Collecter les \'el\'ements du flux dans un conteneur}

Afin de collecter les \'el\'ements d'un flux dans un conteneur, l'\TT{API} fournit la m\'ethode collect d\'ecrite dans le tableau~\ref{methodes_api.tab}. Cette m\'ethode retourne un conteneur \TT{STL}. Le type pour les \'el\'ements du conteneur et le type du conteneur sont donn\'es par les types de param\`etres \TT{template} de la m\'ethode \TT{collect}. Par exemple dans l'exemple du listage~\ref{employeeNamesExample.c++} les param\`etres \TT{template} de la m\'ethode \TT{collect} sont de type \TT{string} et respectivement \TT{list}. Donc, dans notre exemple la m\'ehode \TT{collect} va retourner une liste de type \TT{string} avec les noms d'employ\`ees.
...
\end{lstlisting}
\caption{Extraire les noms d'employ\'ees.}
...
Consiste \`a combiner tous les \'el\'ements du flux en un seul r\'esultat. La m\'ethode \TT{max} d\'ecrite dans le tableau~\ref{methodes_api.tab} est l'une des m\'ethodes offertes par l'\TT{API} qui illustre ce type de fonctionnalit\'e. Cette m\'ethode prend un comparateur en argument pour comparer les \'el\'ements du flux. Le listage~\ref{olderEmployeeExample.c++} montre un exemple ou les \'el\'ements de type \TT{Employee} d'un flux sont compar\'es afin de trouver l'employ\'ee le plus \^ag\'ee.
...
Assez souvent une op\'eration de la base de donn\'ees consiste \`a regrouper ses \'el\'ements dans un ensemble en fonction d'une ou de plusieurs propri\'et\'es. L'\TT{API} fournit une fonctionnalit\'e similaire via la m\'ethode \TT{groupByKey} d\'efinie dans le tableau~\ref{methodes_api.tab}. Le listage~\ref{groupByKeyExample.c++} montre un exemple ou les employ\'ees sont group\'ees par leurs \^ages.
...
\end{lstlisting}
\caption{Grouper les employ\'ees par leurs \^ages.}
...
Une op\'eration de regroupement et r\'eduction peut \^etre verbeuse et source d'erreurs lorsqu'elle est impl\'ement\'ee avec un style imp\'eratif. L'\TT{API} fournit une op\'eration similaire dans un style plus fonctionnel. La m\'ethode \TT{reduceByKey} d\'ecrite dans le tableau~\ref{methodes_api.tab} combine une op\'eration de regroupement et une op\'eration de r\'eduction dans le m\^eme temps. Le programme compte de mots du listage~\ref{wordcount.fig} montre un exemple ou une telle fonctionnalit\'e est tr\`es utile. Les \'el\'ements du flux qui sont les mots contenus dans un fichier sont regroup\'es par mot, puis une op\'eration de r\'eduction est appliqu\'ee \`a l'ensemble de mots au pr\'ealable group\'e. L'op\'eration de r\'eduction \TT{Reducer} p.~\pageref{reducer.sect} est repr\'esent\'ee par l'argument de la m\'ethode. Dans ce cas particulier, elle a le r\^ole de compter l'occurrence de chaque mot. Le r\'esultat retourn\'e par la m\'ethode \TT{reduceByKey} c'est un \TT{map} de type cl\'e valeur. La cl\'e repr\'esente chaque mot distinct du fichier et la valeur repr\'esente l'occurrence du mot dans le fichier.