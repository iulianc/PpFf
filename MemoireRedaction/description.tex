...
\GT{Partout ci-bas dans les extraits de code, j'ai remplac\'e
<<\TT{Flux}>> par <<\TT{Flow}>>!  Et j'ai remplac\'e <<\TT{Flow()}>>
par <<\TT{Flow}>> lorsque c'\'etait suivi d'un appel \`a
\TT{source}. Parce qu'il s'agit d'une m\'ethode statique.  J'ai aussi
modifi\'e un peu la mise en forme, pour que l'appel \`a \TT{::source}
soit coll\'e sur \TT{Flow}. \`A v\'erifier!}

\IC{Une petite n\'egligence de ma part qui fait une grande diff\'erence. Je me rappelle. J'ai remplac\'e Pipe par Flux.}


\begin{figure}
\centering
     \includegraphics[ height=15cm, width=9cm]{Figures/MethodsAPI.jpg}
      \caption{Les diff\'erentes m\'ethodes export\'ees par l'{API},
...
\section{Application \TT{WordCount}}
...
\item L'op\'eration \TT{find} retourne dans le flux seulement les mots qui ne sont pas vides --- par un appel \`a \TT{notEmpty}.
...
\endlastfoot
\hline
	\multicolumn{3}{|c|}{Source --- M\'ethodes {\bf statiques} pour cr\'eer un flux \`a partir d'une source}
    \\
\hline
	\begin{tabular}{@{}l@{}}
...
\section{Sources de donn\'ees: m\'ethodes \TT{source}}


La source de donn\'ees est le premier op\'erateur utilis\'e pour cr\'eer un flux. 
%
...
Les signatures pour les deux m\'ethodes sont donn\'ees dans le tableau~\ref{methodes_api.tab}. Tandis que la premi\`ere m\'ethode, \TT{source(path)}, produit les donn\'ees d'un flux \`a partir de contenu (lignes) d'un fichier, la deuxi\`eme m\'ethode, \TT{source(begin\_iterator, end\_iterator)}, produit les donn\'ees d'un flux \`a partir des \'el\'ements d'un conteneur STL.
...
\subsection{Find}
Une op\'eration courante consiste \`a d\'eterminer si certains \'el\'ements d'un ensemble de donn\'ees satisfont une certaine propri\'et\'e. L'API de \TT{PpFf} fournit une telle fonctionnalit\'e via la m\'ethode \TT{find}. D\'ecrite dans le tableau~\ref{methodes_api.tab}, la m\'ethode \TT{find} s\'electionne les \'el\'ements d'un flux selon un pr\'edicat de sorte que seuls les \'el\'ements qui  satisfont le pr\'edicat sont envoy\'es \`a l'\'etape suivante. \`A noter qu'il est obligatoire que le pr\'edicat retourne une expression bool\'eenne. 
...
Un op\'erateur de classe \TT{Reducer} est utilis\'e pour sp\'ecifier comment r\'eduire les \'el\'ements d'un flux \`a une valeur unique. Le listing~\ref{reducer.c++} présente la spécification  de la classe \TT{Reducer} avec les signatures des trois constructeurs associ\'es. 
