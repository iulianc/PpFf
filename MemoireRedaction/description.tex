...
\GT{J'ai supprimé la phrase suivante~: <<\TT{PpFF} facilite le
d\'eveloppement d'applications de traitement de donn\'ees gr\^ace \`a
son expressivit\'e.>> Parce que c'est une affirmation forte --- ça
rend ça facile --- alors que oui, c'est vrai par rapport à FastFlow,
mais pas nécessairement par rapport à Java.  Et le fait de faire des
affirmations parfois fortes, sans qu'elles soient validées fortement
ensuite, était quelque chose qui avait agacé les évaluateurs ---
qu'ils avaient mentionné explicitement lors de la rencontre Zoom fait
pour discuter du résultat de la première version!}
...
% L'avantage de la m\'ethode \TT{source} de \TT{PpFf} par rapport \`a
% la m\'ethode \TT{stream} de \TT{Java} est que pour la m\'ethode
% \TT{source} de \TT{PpFf}, l'\'el\'ement de d\'ebut et l'\'elément de
% fin de la collection sont sp\'ecifiés dans l'argument de la
% m\'ethode. Par exemple, on peut traiter seulement certains
% \'el\'ements de cette collection et pas tous les \'el\'ements comme
% dans le cas de \TT{Java}.
%
