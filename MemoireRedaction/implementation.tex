...
\GT{On ne peut pas dire que tu décris la création et l'exécution, car
dans ce cas il faudrait que tu utilises des diagrammes de séquence et
non pas des diagrammes de classes. Ces derniers diagrames permettent
de montrer la structure, statique, des classes. L'aspect dynamique ici
n'est illustrée que par le fait qu'il y a deux versions de chaque
diagramme: avant vs.\ après l'appel à \TT{build\_ff\_node()}.}
...
\GT{Pour l'exemple 3, tu peux \^etre plus bref et ne mentionner le
fait que tu illustres le cas où deux séries de farm on un nombre de
