...
\begin{itemize}
	\item L'opération \TT{linesFromFile} renvoie un flux séquentiel de lignes à partir d'un fichier~; le fichier est identifi\'e par le paramètre \TT{path} fourni en argument;

	\item L'op\'eration \TT{find} s\'electionne dans le flux les mots qui ne sont pas vides;

	\item L'op\'eration \TT{parallel} indique que les op\'erations subs\'equentes doivent \^etre ex\'ecut\'ees en parall\`ele;
	
	\item L'op\'eration \TT{map} transforme les lettres majuscules du mot en minuscules;
	
	\item Finalement, l'op\'eration \TT{collect} combine tous les \'el\'ements r\'esultants dans un conteneur de type \TT{vector}.
\end{itemize}
...
Dans un \TT{Pipeline} le premier op\'erateur est toujours un op\'erateur d'entr\'ee, lequel est impl\'ement\'e sous forme d'un \TT{ff\_node}. Dans notre exemple, \TT{linesFromFile} renvoie un flux s\'equentiel de lignes \`a partir d'un fichier. De plus, l'op\'erateur d'entr\'ee a le r\^ole d'envoyer un signal \TT{EOS} lorsque la g\'en\'eration d'entr\'ee est termin\'ee. \TT{EOS} instruit les nœuds \TT{ff\_node}s de \TT{FastFlow} de finir leur ex\'ecution apr\`es la r\'eception de celui-ci.
