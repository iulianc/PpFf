...
\caption{Statistiques sur la mise en \oe uvre~: les modules et le fichier \TT{Flow.hpp}.\label{statistiquesPpFf.tab}}\\
\hline
...
\GT{Je ne comprends toujours pas la phrase qui pr\'ec\`ede --- la derni\`ere partie en vert. \`A r\'eviser!}

\IC{La derni\`ere fois, j'ai chang\'e la phrase suivante et non la pr\'ec\'edente. Cette fois, j'ai chang\'e la bonne phrase.}
...
\textbf{Nombre de m\'ethodes de l'API} & \textbf{Nombre de cas de tests} & \textbf{Nombre d'assertions}\\
\hline
%\endhead
\hline
	21 &
...
\GT{Ci-bas: il faudrait indiquer l'URL du site web de catch.}

\IC{J'ai ajout\'e l'URL pour catch.}
...
Chaque m\'ethode de l'API a son propre fichier de test contenant plusieurs sc\'enarios de test. Au total, les 21 m\'ethodes expos\'ees \`a l'utilisateur sont test\'ees avec 123 cas de tests. Plusieurs assertions sont g\'en\'er\'ees pour chaque sc\'enario de test. En nombre de 13 895, le nombre d'assertions est beaucoup plus grand que le nombre de cas de tests. Elles sont n\'ecessaires pour tester la structure de donn\'ees.


\GT{Il faudrait expliquer pourquoi le nombre d'assertions est beaucoup plus grand que le nombre de cas de tests: utilisation de boucles pour tester des structures de donn\'ees, etc.}.

\IC{J'ai ajout\'e quelques mots pour expliquer la diff\'erence entre le nombre de cas de tests et le nombre d'assertions.}
