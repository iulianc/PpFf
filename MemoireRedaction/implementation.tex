...
Cette section d\'ecrit le mod\`ele d'ex\'ecution de \TT{PpFf} \`a l'aide du code source de trois petits exemples. L\'eg\`erement diff\'erents, les exemples montrent \`a l'aide de diagrammes les \'etapes de cr\'eation et d'exécution du flux ainsi que la relation entre les deux biblioth\`eques \TT{PpFf} et \TT{FastFlow}.
...
\begin{itemize}
\item \TT{source}, qui g\'en\`ere un flux s\'equentiel de nombres entiers \`a partir d'un conteneur.
\item \TT{max}, qui trouve le nombre maximum d'\'el\'ements du conteneur. 
\end{itemize}
...
\centering
      ...
      \caption{La structure interm\'ediaire \TT{PpFf}.}
       \label{objets1-ppff.fig}
\end{figure}


\TT{PpFf} construit tout d'abord une repr\'esentation interm\'ediaire, puis g\'en\`ere ensuite un graphe de n\oe{}uds \TT{FastFlow}. La repr\'esentation interm\'ediaire, illustr\'ee dans la figure~\ref{objets1-ppff.fig}, est compos\'ee des objets du type \TT{Pipeline} et \TT{Node}. Le premier objet cr\'e\'e est \TT{pipe}, une instance de la classe \TT{Pipeline}. Au fur et \`a mesure que les m\'ethodes sont enchain\'ees dans le \TT{Flow}, \TT{PpFf} construit la structure en ajoutant au \TT{Pipeline} des instances du type \TT{Node}. On peut observer que pour les deux m\'ethodes enchain\'ees dans \TT{Flow}, deux instances du type \TT{Node} sont ajout\'ees au \TT{Pipeline} : \TT{node0} et \TT{node1}. 

Selon la m\'ethode choisie par l'utilisateur, chaque \TT{Node} garde une r\'ef\'erence \`a une instance du type \TT{Operator}. Dans notre cas, les deux objets \TT{Operator} sont \TT{sourceOp}, une instance de la classe \TT{SourceOperator} et \TT{maxOp} une instance de la classe \TT{MaxOperator}. 


\begin{figure}
\centering
         \includegraphics[width=1.0\textwidth]{Figures/objets1-ff.png}
      \caption{La structure interm\'ediaire \TT{PpFf} et le graphe FastFlow appropri\'e.}
       \label{objets1-ff.fig}
\end{figure}
 
\'Etant donn\'e que le dernier op\'erateur de \TT{Pipeline}, \TT{maxOp}, est un op\'erateur de type collecteur, cela lancera la construction du graph \TT{FastFlow} appropri\'e. Illustr\'e dans la figure~\ref{objets1-ff.fig}, la structure \TT{FastFlow} est cr\'e\'ee en parcourant chaque \TT{Node} de la structure interm\'ediaire. On peut observer que \TT{pipe} g\'en\`ere un objet \TT{ff\_pipeline} et les deux instances \TT{Node} retournent les deux op\'erateurs : \TT{sourceOp} et \TT{maxOp} qui sont d'objets de type \TT{ff\_node}, une classe de \TT{FastFlow}. La nouvelle structure ainsi cr\'e\'ee, sera ex\'ecut\'ee automatiquement et le r\'esultat du traitement, dans ce particulaire cas, le nombre maximum d'\'el\'ements d'un conteneur, est retourn\'e au \TT{Pipeline} par l'interm\'ediaire d'objet \TT{collector}. 
...
Les figures~\ref{objets2-ppff.fig} et~\ref{objets2-ff.fig} ...
