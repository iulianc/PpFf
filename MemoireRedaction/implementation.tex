...
\GT{Je crois qu'il serait bon d'indiquer quelque part, dans le texte
ou dans le caption, pourquoi il y a une différence au niveau des
couleurs des objets: p\^ale = objet PpFf seulement, foncé = objet
FastFlow.  Probablement préférable dans le texte, lors de la
présentation de la figure 3.8, en soulignant que cette distinction au
niveau des couleurs se retrouve aussi dans les autres exemples.}

\IC{J'ai indiqué lors de la présentation de la figure 3.8 pourquoi il y a une différence au niveau des couleurs des objets.}
...
%
Les objets composant la structure interm\'ediaire sont distingu\'es par une couleur diff\'erente de ceux composant le graphe \TT{FastFlow}. P\^ale pour les objets \TT{PpFf} et fonc\'e pour les objets \TT{FastFlow}, est la distinction au niveau des couleurs qui se retrouve aussi dans les autres exemples. 
%
On peut observer que \TT{pipe} g\'en\`ere un objet \TT{ff\_pipeline} et que les deux instances de \TT{Node} retournent les op\'erateurs associés, \TT{sourceOp} et \TT{maxOp}, qui sont d'objets de type \TT{ff\_node}, la classe de base d'un graphe \TT{FastFlow}. La structure ainsi cr\'e\'ee sera ex\'ecut\'ee automatiquement en appelant la méthode \TT{run\_and\_wait\_end()} de FastFlow, et le r\'esultat du traitement, dans ce cas l'élément maximum, sera retourn\'e au \TT{Pipeline} par l'interm\'ediaire de l'objet \TT{collector}. 
...
On a vu dans l'exemple~1 que lorsqu'un op\'erateur de type collecteur, \TT{maxOp} dans notre cas, est ajout\'e au {pipeline}, ceci lance la construction du graphe \TT{FastFlow} associ\'e via la m\'ethode \TT{build\_ff\_node}. Illustr\'ee dans la figure~\ref{objets2-ff.fig}, le graphe ainsi g\'en\'er\'e est compos\'e des objets \TT{ff\_pipeline}, \TT{ff\_node} et \TT{ff\_farm} correspondant aux instances de \TT{PpFf} \TT{pipe} (classe \TT{Pipeline}), \TT{sourceOp} (classe \TT{Node}) et \TT{farm} (classe \TT{Farm}). La nouvelle structure cr\'e\'ee sera ex\'ecut\'ee en appelant la m\'ethode \TT{run\_and\_wait\_end()} de \TT{FastFlow}. Par rapport \`a l'exemple~1, l'instance de \TT{Farm} divise le flux en deux sous-flux et les maximums produits par les sous-flux sont ensuite combin\'es par l'objet \TT{collector}. Ceci est illustr\'e dans la figure~\ref{objets2-ff.fig}, o\`u le \TT{collector} garde une r\'ef\'erence aux deux op\'erateurs, \TT{maxOp1} et \TT{maxOp2}, qui d\'eterminent le maximum pour chaque sous-flux. La combinaison des r\'esultats par le \TT{collector} se fait par l'interm\'rdiaire de l'opérateur <<+=()>>. Le code source pour la classe \TT{Collector} est pr\'esent\'e dans le listing~\ref{Collector.c++}. Lorsque la m\'ethode \TT{value()} de cette classe est appel\'ee, les op\'erateurs gard\'es comme r\'ef\'erence, l'un pour chaque sous-flux, sont combin\'es et le r\'esultat final est retourn\'e au \TT{Pipeline}.  


\GT{Ci-haut: je crois qu'il faudrait rappeler que la combinaison des
résultats par le collector se fait par l'intermédiaire de l'opérateur
"+=()", lequel est appelé par la méthode value(). Peut-être
faudrait-il même présenter le code de la méthode value() de la classe
Collector?}

\IC{J'ai ajouté le code source de la classe Collector dans la description de l'exemple 2}
