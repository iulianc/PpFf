...
\gt{Je crois qu'ici, il serait bien de donner une id\'ee de {\bf
l'envergure} de ta mise en oeuvre~: combien de fichiers et de lignes
de code pour la sp\'ecificatioan de l'API?  nombre de fichiers, lignes
de code et nombre de tests pour les tests unitaires et tests
fonctionnels (programmes complets).  Je trouve important de donner une
id\'ee de \c{c}a, pour montrer que le travail r\'ealis\'e est loin
d'\^etre trivial (quantit\'e de code), et aussi qu'il a \'et\'e fait
de fa\c{c}on <<professionnelle>>, avec de nombreux tests unitaires
pour assurer que les diverses m\'ethodes de l'API produisent les bons
r\'esultats.}

\ic{J’ai ajouté quelques mots concernant la statistique. Est-ce que vous croyez que je dois ajouter plus de détails sur les tests?}

\begin{center}
\footnotesize
\begin{longtable}{|l|r|r|r|r|r|r|}
\caption{Statistiques sur la mise en \oe uvre --- modules et fichier \TT{Flow.hpp}.\label{statistiquesPpFf.tab}}\\
\hline
...
\gt{Pas besoin, il me semble, de r\'ep\'eter le nombre de lignes ou
fichiers pour chaque module. Peut-\^etre juste le plus gros.}

Cette section donne un aper\c {c}u de l'envergure du projet r\'ealis\'e. \TT{PpFf} est divis\'e en plusieurs modules. Le tableau~\ref{statistiquesPpFf.tab} pr\'esente quelques statistiques sur la mise en \oe uvre~: nombre de fichiers et nombre de lignes de code dans chaque module. Il faut noter que les lignes blanches et les lignes de commentaires ont \'et\'e prises en compte lors du décompte de lignes de code.

\GT{Il faudrait clarifier ce que tu as compt\'e dans les lignes de
code: les lignes blanches aussi? les lignes de commentaires?  C'est ok
de les compter, mais il faut juste l'indiquer clairement.}

\IC{Les lignes blanches et les lignes de commentaires ont \'et\'e prises en compte. Je les ai compt\'ees automatiquement \`a l'aide d'un programme en c++.}

Le module \TT{pipeline} est le c\oe ur du traitement des flux~: il g\`ere la cr\'eation des \TT{pipeline}s, le traitement en parall\`ele et la communication entre les n\oe uds composant un \TT{pipeline}~; il g\`ere aussi la communication entre \TT{FastFlow} et \TT{PpFf}.

Le plus grand module du projet est  \TT{operators}. Totalisant 1225 de lignes de code, ce module d\'efinit la logique de traitement de chacune des op\'erations de l'\TT{API}. 

Le module \TT{stages} g\`ere le groupement logique d'une ou plusieurs op\'erations d'une \'etape dans la cha\^ine de traitement d'un \TT{pipeline}. 

Tout projet a besoin d'objets et de fonctionnalit\'es qui sont partag\'es par d'autres modules. Dans \TT{PpFf}, ces objets sont regroup\'es dans le module \TT{utilities}, le plus petit module de l'\TT{API}.

Le dernier module dans le traitement du flux, le module \TT{collections}, g\`ere l'acc\`es au regroupement logique des \'el\'ements n\'ecessaires au traitement interne de l'\TT{API}.

\GT{Je ne comprends pas la phrase qui pr\'ec\`ede. \`A r\'eviser!}

\IC{J’ai reformulé la phrase.}

Les m\'ethodes permettant \`a l'utilisateur de manipuler des flux de donn\'ees sont regroup\'ees dans le fichier Flow.hpp. Avec 345 lignes de code, ce fichier est le point d'entr\'ee de l'API. Plus pr\'ecis\'ement, c'est l'interface utilisateur de \TT{PpFf}. Chaque fonctionnalit\'e d\'efinie dans cette interface est minutieusement test\'ee avec des tests unitaires. L'outil utilis\'e pour les tests unitaire c'est \TT{catch}. %
%
\footnote{Un cadre de test en C++ pour les tests unitaires. Avec seulement un fichier en t\^ete, il peut \^etre utilis\'e avec l'approche TDD--Test Driven Development ou BDD-Behavior Driven Development.}
%
Le tableau~\ref{statistiquesUnitTest.tab} pr\'esente quelques statistiques sur les tests cr\'e\'es.

\begin{center}
\footnotesize
\begin{longtable}{|r|r|r|}
\caption{Statistiques sur les tests unitaires.\label{statistiquesUnitTest.tab}}\\
\hline

\textbf{Nombre de m\'ethodes de l'API} & \textbf{Nombre de cas de tests} & \textbf{Nombre de assertions}\\
\hline
%\endhead
\hline
	21 &
...
Chaque m\'ethode de l'API a son propre fichier de test contenant plusieurs sc\'enarios de test. Au total, les 21 m\'ethodes expos\'ees \`a l'utilisateur sont test\'ees avec 123 de cas de tests en utilisant 13895 d'assertions.
