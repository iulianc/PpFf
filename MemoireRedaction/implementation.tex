...
\centering
      ...
      \caption{Le diagramme UML d\'etaill\'e de \TT{Pipeline}.}
       \label{pipeline.fig}
\end{figure}
...
\GT{Il faudrait que dans cette figure 3.3., la partie Opertors soit
dissimulée --- donc avec juste la classe Operator et le package
Operators -- et ce pour ainsi mieux mettre l'accent sur le package
Pipeline. Sinon, 3.3 et 3.4 sont trop semblables, avec presque autant
de détails dans l'une et l'autre figure.}

\IC{Ça a l'air beaucoup mieux.}
...
\centering
      ...
      \caption{Le diagramme UML d\'etaill\'e de \TT{Operators}.}
       \label{operators.fig}
\end{figure}


Dans la terminologie de \TT{PpFf}, chaque m\'ethode expos\'ee par son interface repr\'esente une op\'eration et chaque op\'eration est impl\'ement\'ee par un op\'erateur. Plus pr\'ecisément, les op\'erateurs repr\'esentent les \'etapes dans la chaine de traitement de donn\'ees. D\'ecrit dans la figure~\ref{operators.fig}, les op\'erateurs h\'erite de la classe de base, \TT{Operator}. Ils peuvent \^etre class\'es en trois cat\'egories : source, interm\'ediaire et collecteur.

Un \TT{pipeline} est compos\'e d'un op\'erateur source, de z\'ero ou plusieurs op\'erateurs interm\'ediaires et d'un op\'erateur collecteur. L'op\'erateur source est le premier op\'erateur ajout\'e dans le flux. Sans un tele op\'erateur le traitement ne peut avoir lieu.

Les op\'erateurs interm\'ediaires tels que \TT{find} ou \TT{map} sont des op\'erateurs sans \'etat. Autrement dit, l'\'etat du traitement n'est pas conserv\'e. Ce qui est important, c'est que les op\'erateurs interm\'ediaires n'effectuent aucun traitement tant qu'un op\'erateur collecteur n'est pas ajout\'e au \TT{pipeline}. 

L'op\'erateur collecteur, l'op\'erateur avec \'etat, est le dernier op\'erateur ajout\'e au \TT{pipeline}. L'ajout de cet op\'erateur entra\^ine l'ex\'ecution du traitement. La valeur r\'esultante du traitement est conserv\'ee en interne par chaque op\'erateur. C'est cette valeur qui va \^etre retourn\'ee au \TT{Pipeline}.

La fonctionnalit\'e sp\'ecifique \`a chaque op\'eration de \TT{pipeline} est impl\'ement\'ee par la m\'ethode \TT{svc} de chaque op\'erateur. Comme on peut le voir dans la figure~\ref{operators.fig}, un \TT{Operator} h\'erite de la classe abstraite \TT{ff\_node} de \TT{FastFlow}. De cette fa\c {c}on, chaque op\'erateur surcharge la m\'ethode \TT{svc} qui est appel\'ee automatiquement au moment de l'ex\'ecution du flux. Cela nous permet d'ajouter la fonctionnalit\'e d\'esir\'ee au corps de cette m\'ethode. 
...
\GT{Même remarque que ci-bas: L'utilisation des $S_i$ n'est pas
vraiment utile, ne fait que rendre ça plus complexe. Donc, supprime
les $S_i$ de la figure!}

\IC{$S_i$ vienne de l'ancien stage. Bonne observation.}

\centering
     \includegraphics[width=1.0\textwidth]{Figures/ParallelismeDuFlux.png}
...
\GT{Je suggère que dans cette figure, tu supprime les $S_1$, etc. Ça
n'apporte rien aux explications, au contraire, ça rend cela plus
compliqué: opération, worker, étape....}

\IC{J'ai supprimé les $S_i$}

\centering
     \includegraphics[width=1.0\textwidth]{Figures/DataParallelisme.png}
