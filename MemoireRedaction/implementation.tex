...
\GT{Tu as omis le e final dans \TT{ff\_node}!}

\GT{Dans le diagramme tu indiques <<operator+>>.  Idem dans le texte
ci-bas (dernier paragraphe).  Par contre, dans le code, c'est
l'op\'erateur <<operator+=>> qui est pr\'esent\'e!}
...
\IC{J'ai mis \`a jour la figure et le texte.}
...
%Chaque nœud final d'un \TT{pipeline} (classe \TT{Node} de \TT{Ppff}) h\'erite de \TT{ff\_node} et de la classe \TT{BaseCollector}. La figure~\ref{InheritanceCollectorOperator.fig} montre la structure de classes. L'\TT{operator+=} d\'efini dans la classe \TT{BaseCollector} est surcharg\'e (\emph{overrided}) dans chaque \TT{Node}. De cette fa\c {c}on, chaque \TT{Node} qui h\'erite de cette classe peut impl\'ementer sa propre fonctionnalit\'e. Lorsque le \TT{pipeline} termine son traitement, la classe \TT{Stage} compose le r\'esultat final en appelant l'op\'erateur \TT{operator+=} de chaque \TT{Node} dans une boucle. Le listing~\ref{ImplementationOverridingOperator} montre l'impl\'ementation de l'\TT{operator+=} dans l'un des op\'erateurs finaux de \TT{PpFf}, \TT{Collector}. Dans ce cas, le r\'esultat partiel produit par chaque thread est ajout\'e dans un conteneur~STL, cr\'e\'e dans la classe \TT{Stage}. \ldots \GT{dire o\`u ce container est cr\'e\'e!} \IC{J\'e complet\'e la phrese.}

\IC{J'ai mis en commentaire le texte pour l'instant. Je ne sais pas de quoi parler. Je dois r\'efl\'echir.}
