...
\GT{Ne pas confondre: nombre maximum d'éléments vs.\ l'élément maximum~;)} 

\IC{Mon Dieu! Le sens de l'expression change complètement.}

\end{itemize}


\GT{J'ai réduit la taille, pour que les deux apparaissent sur la même page! Faire de même avec figure~2, si c'est possible, pas trop petit.}

\IC{J'ai fait la même chose avec les figures~2, mais elles ne veulent pas apparaître sur une seule page.}
...
\GT{Ci-bas: il faut que tu indiques plus explicitement que la création
du graphe se fait via \TT{build\_ff\_node} et rappeler qu'ensuite c'est
l'opération \TT{run\_and\_wait\_end()} de FastFlow qui est lancée.}

\IC{J'ai indiqué la méthode buil\_ff\_node.}
 
\'Etant donn\'e que le dernier op\'erateur du {pipeline}, \TT{maxOp}, est un op\'erateur de type collecteur, cela lance la construction du graph \TT{FastFlow} appropri\'e via la m\'ethode \TT{build\_ff\_node}. Illustr\'ee dans la figure~\ref{objets1-ff.fig}, la structure \TT{FastFlow} est cr\'e\'ee en parcourant chaque \TT{Node} de la structure interm\'ediaire. On peut observer que \TT{pipe} g\'en\`ere un objet \TT{ff\_pipeline} et que les deux instances de \TT{Node} retournent les op\'erateurs associés, \TT{sourceOp} et \TT{maxOp}, qui sont d'objets de type \TT{ff\_node}, une classe de \TT{FastFlow}. La structure ainsi cr\'e\'ee sera ex\'ecut\'ee automatiquement en appelant la méthode \TT{run\_and\_wait\_end()} de FastFlow, et le r\'esultat du traitement, dans ce cas l'élément maximum, sera retourn\'e au \TT{Pipeline} par l'interm\'ediaire de l'objet \TT{collector}. 
...
Le deuxi\`eme exemple \'etudi\'e, analyse le fonctionnement parall\`ele en utilisant les instances parall\`eles d'un \TT{farm}. Le code source est montr\'e dans le listing~\ref{objets2.c++}. Par rapport au premier exemple, il y a une op\'eration de plus. L'op\'eration introduite dans le flux par cha\^inage de la m\'ethode \TT{parallel(2)}, indique de r\'epartir les \'el\'ements du flux entre deux (2) instances parall\`eles d'un \TT{farm}. Cette op\'eration cr\'ee et ajoute une instance du type \TT{Farm} au \TT{Pipeline}. Ceci peut \^etre observ\'e dans la structure interm\'ediaire cr\'e\'ee par \TT{PpFf} et illustr\'ee dans la figure~\ref{objets2-ppff.fig}.

\begin{figure}
\centering
         \includegraphics[width=0.66\textwidth]{Figures/objets2-ppff.png}
...
\begin{figure}
\centering
         \includegraphics[width=0.66\textwidth]{Figures/objets2-ff.png}
      \caption{La structure interm\'ediaire \TT{PpFf} et le graphe FastFlow associ\'e pour l'exemple~2.}
       \label{objets2-ff.fig}
\end{figure}

On rappelle que, lorsqu'une instance de \TT{Farm} est ajout\'ee au \TT{pipeline}, le flux est divis\'e en sous-flux --- appel\'es les instances parall\`eles d'un \TT{farm} --- o\`u chaque sous-flux est une instance de la classe \TT{Worker}. Dans notre cas les deux sous-flux sont représent\'es dans la figure~\ref{objets2-ppff.fig} par les instances \TT{worker0} et \TT{worker1} o\`u chacune de ces instances garde une r\'ef\'erence \`a un objet du type \TT{MaxOperator}. 

On a vu dans l'exemple~1 que, lorsqu'un op\'erateur de type collecteur, \TT{maxOp} dans notre cas, est ajout\'e au \TT{Pipeline}, lancera la construction du graphe \TT{FastFlow} appropri\'e via la m\'ethode \TT{build\_ff\_node}. Illustr\'ee dans la figure~\ref{objets2-ff.fig}, le graph ainsi g\'en\'er\'e est compos\'e des objets \TT{ff\_pipeline}, \TT{ff\_node} et \TT{ff\_farm} correspondant aux instances de \TT{PpFf} \TT{pipe} (classe \TT{Pipeline}), \TT{sourceOp} (classe \TT{Node}) et \TT{farm} (classe \TT{Farm}). La nouvelle structure cr\'e\'ee sera ex\'ecut\'ee automatiquement en appelant la m\'ethode \TT{run\_and\_wait\_end()} de \TT{FastFlow}. Par rapport \`a l'exemple~1, l'instance de \TT{Farm} divise le flux en deux sous-flux et le maximum de chaque sous-flux r\'esultant est combin\'e par l'objet \TT{collector}. Ceci peut \^etre observ\'e dans la figure~\ref{objets2-ff.fig} o\`u le \TT{collector} garde une r\'ef\'erence aux deux op\'erateurs, \TT{maxOp1} et \TT{maxOp2}, qui d\'eterminent le maximum pour chaque sous-flux.   
...
Le dernier exemple \'etudi\'e analyse le fonctionnement parall\`ele en utilisant deux s\'eries de \TT{farm}. Le code source est montr\'e dans le listing~\ref{objets3.c++}. Les deux s\'eries de \TT{farm} sont introduites dans le flux en encha\^inant la m\'ethode \TT{parallel}, l'une avec l'argument~2 et l'autre avec l'argument~4, indiquant que les \'el\'ements du flux sont r\'epartis entre deux (2) et quatre (4) instances parall\`eles d'un \TT{farm}. Par rapport au exemple~2, une autre m\'ethode a \'et\'e ajout\'ee au flux. La m\'ethode \TT{map}, une m\'ethode d'instance, multiplie chaque \'el\'ement du flux avec la valeur 3.
...
La structure interm\'ediaire cr\'e\'ee par \TT{PpFf}, figure~\ref{objets2-ppff.fig}, contienne deux instances du type \TT{Farm}, \TT{farm0} et \TT{farm1}. On rappelle que, toutes les op\'erations suivantes \`a la m\'ethode \TT{parallel} (l'instance \TT{Farm} de \TT{PpFf}) sont ex\'ecut\'ees en parall\`ele dans des sous-flux distincts. Dans notre cas, les op\'erations introduites par les m\'ethodes \TT{map} et \TT{max} seront ex\'ecut\'ees en parall\`ele dans diff\'erentes sous-flux --- instances du type \TT{Worker}. Ceci peut \^etre observ\'e dans la figure~\ref{objets3-ppff.fig} o\`u chaque \TT{Worker} de l'objet \TT{farm0} garde une r\'ef\'erence \`a un objet du type \TT{MapOperator} et chaque \TT{Worker} de l'objet \TT{farm1} garde une r\'ef\'erence \`a un objet du type \TT{MaxOperator}. 

Comme dans les deux autres exemples, le graph \TT{FastFlow} sera ex\'ecut\'e automatiquement apr\'es qu'il est construit via la m\'ethode \TT{build\_ff\_node}, m\'ethode appel\'ee lors de l'ajout d'un op\'erateur du type collecteur dans le flux. Ce graphe est montr\'e dans la figure~\ref{objets3-ff.fig}. Le r\'esultat final, l'\'el\'ement maximum du flux, est d\'etermin\'e par l'objet \TT{collector} en combinat les r\'esultats partiels de chacun de quatre sous-flux de l'objet \TT{farm1}.  
