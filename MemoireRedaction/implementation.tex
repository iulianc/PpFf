...
\section{Impl\'ementation de \TT{PpFf} \`a l'ex\'ecution}
...
Cette section d\'ecrite le mod\`ele d'ex\'ecution de l'API \`a l'aide du code source d'une application \TT{PpFf}. Illustr\'ee dans le listing~\ref{listingExampleImplementationPpFf}, l'application est compos\'ee de quatre op\'erations :
\begin{itemize}
...
La classe \TT{Pipeline} de l'API repr\'esente l'entit\'e principale de l'application. \`A l'ex\'ecution, le \TT{Pipeline} correspond \`a la classe \TT{ff\_pipeline} de \TT{FastFlow}. Un \TT{Pipeline} est compos\'e de plusieurs nœuds de type \TT{ff\_node} ou \TT{ff\_farm}. La figure~\ref{ExempleRuntimeExecution.fig} montre la repr\'esentation graphique d'un \TT{Pipeline} compos\'e de quatre op\'erateurs d\'ecrits au d\'ebut de cette section.
 
Dans un \TT{Pipeline} le premier op\'erateur est toujours un op\'erateur d'entr\'ee. Cela est impl\'ement\'e comme \TT{ff\_node}. Dans notre exemple, \TT{linesFromFile} renvoie un flux s\'equentiel de lignes \`a partir d'un fichier. De plus, l'op\'erateur d'entr\'ee a le r\^ole d'envoyer un signal \TT{EOS} lorsque la g\'en\'eration d'entr\'ee est termin\'ee. \TT{EOS} instruit les nœuds \TT{ff\_nodes} de \TT{FastFlow} de finir leur ex\'ecution apr\`es la r\'eception de celui-ci.

La classe repr\'esentant chaque nœud dans un \TT{Pipeline} contient les informations concernant son r\^ole dans la chaine du traitement. Par exemple l'op\'erateur \TT{find} du listing~\ref{listingExampleImplementationPpFf}, a le r\^ole de s\'electionner dans le flux seulement les mots qui ne sont pas vides. Ce r\^ole est d\'efini par utilisateur \`a la cr\'eation du \TT{Pipeline}. 


Un \TT{Pipeline} est toujours ex\'ecut\'e en parall\`ele. Nous rappelons que, dans \TT{FastFlow}, chaque \TT{ff\_node} est un thread. Le mod\`ele de parall\'elisme de t\^aches est appliqu\'e par d\'efaut. Lorsque la m\'ethode parall\`ele est ajout\'ee dans la chaine du traitement d'un Pipeline, les op\'erateurs seront ex\'ecut\'es en parall\`ele en appliquant le mod\`ele de parall\'elisme de donn\'ees. Dans l'exemple du listing~\ref{listingExampleImplementationPpFf}, les deux premiers op\'erateurs sont ex\'ecut\'es en parall\`ele en appliquant le mod\'ele de parall\'elisme de t\^aches et le mod\`ele de parall\'elisme de donn\'ees sur les derniers op\'erateurs. Dans l'impl\'ementation de \TT{PpFf} le parall\'elisme de donn\'ees est mis en œuvre avec la classe \TT{ff\_farm} de \TT{FastFlow}. La cr\'eation du flux de traitement en utilisant le mod\`ele de parall\'elisme de donn\'ees est simple. Lorsque la m\'ethode \TT{parallel()} est invoqu\'ee dans l'API, une classe de type \TT{ff\_farm} est ajout\'ee dans le \TT{pipeline}. Pour chaque \TT{worker} de \TT{ff\_farm}, un nouveau \TT{ff\_pipeline} est cr\'e\'e. Les op\'erateurs ajout\'es dans la chaine du traitement de l'API seront ajout\'es comme nœuds dans les nouveaux \TT{ff\_pipeline} cr\'e\'es. Par exemple les op\'erateurs \TT{map} et \TT{collect} de la figure~\ref{ExempleRuntimeExecution.fig} sont ajout\'es dans le \TT{pipeline} de chaque \TT{worker} de \TT{ff\_farm}. 

L'ex\'ecution parall\`ele d'un flux est r\'ealis\'ee par l'ex\'ecution de la m\'ethode \TT{run\_and\_wait\_end()} de la classe \TT{ff\_pipeline} de \TT{FastFlow}. Chaque nœud \TT{ff\_node} de \TT{ff\_pipeline} est une classe C++ qui entre dans une boucle infinie via sa m\'ethode \TT{svc()} o\`u:
\begin{itemize}
...
Les nœuds d'un pipeline sont ex\'ecut\'es jusqu'au moment qu'ils re\c coivent le signal \TT{EOS} (End-Of-Stream). \`A ce moment-l\`a, tous les nœuds seront d\'etruits. Dans le cas du traitement du mod\`ele de parall\'elisme de donn\'ees, le r\'esultat global est obtenu en combinant les r\'esultats du traitement de chacun des sous-flux trait\'es dans un fil d'ex\'ecution ind\'ependant. Par exemple chaque op\'erateur \TT{collect} repr\'esent\'e dans la figure~\ref{ExempleRuntimeExecution.fig} collecte le r\'esultat du traitement de chacun des sous-flux trait\'es dans un fil d'ex\'ecution ind\'ependant. Le r\'esultat global est obtenu via la classe \TT{Stage} qui combine ces r\'esultats interm\'ediaires.