...
%
\footnote{Politique \emph{round robin}~: un algorithme pour r\'epartir la charge du travail entre les \emph{threads} de \TT{Farm} 
\GT{Dire quelques mots sur ce que cela signifie.}
\IC{J'ai ajout\'e quelques mots pour la signification de \emph{round robin}}} 
%
...
\centering
     \includegraphics[width=0.8\textwidth]{Figures/Stages.jpg}
      \caption[Le groupement logique de $n$ op\'erations $O_1, O_2, \ldots, O_n$ dans un \TT{stage} S.]{Le groupement logique de $n$ op\'erations $O_1, O_2, \ldots, O_n$ dans un \TT{stage} S.  L'\emph{Emitter} E distribue les \'el\'ements du flux aux diff\'erents fils d'ex\'ecution $W_1, W_2, \ldots, W_n$.}
       \label{Stages.fig}
\end{figure}
...
\centering
     \includegraphics[width=1.0\textwidth]{Figures/StructureCollectorOperator.jpg}
      \caption{Une repr\'esentation graphique de la structure d'un op\'erateur final de \TT{PpFf}.}
...
Les nœuds d'un pipeline sont ex\'ecut\'es jusqu'\`a ce qu'ils re\c coivent le signal \TT{EOS}. \`A ce moment, tous les nœuds sont d\'etruits. Dans le cas de noeuds multiples associ\'es \`a du parall\'elisme de donn\'ees, le r\'esultat global de l'op\'eration est obtenu en combinant les r\'esultats des sous-flux produits par les divers fils d'ex\'ecution. Par exemple, chaque op\'eration \TT{collect} repr\'esent\'ee dans la figure~\ref{ExempleRuntimeExecution.fig} collecte le r\'esultat du traitement de chacun des sous-flux trait\'es dans un fil d'ex\'ecution ind\'ependant. Le r\'esultat global est obtenu via la classe \TT{Stage} qui combine ces r\'esultats interm\'ediaires. 

Chaque nœud final de \TT{pipeline} (classe Node de \TT{Ppff}) h\'erite de \TT{ff\_node} et de la classe \TT{BaseCollector}. La figure~\ref{InheritanceCollectorOperator.fig} montre la repr\'esentation graphique de cette structure. L'\TT{operator+} d\'efini dans la classe \TT{BaseCollector} est surcharge (overriding en anglais) dans chaque \TT{Node}. De cette fa\c {c}on les \TT{Node} qui h\'eritent de cette classe impl\'ementent sa propre fonctionnalit\'e. Lorsque le \TT{pipeline} finit son traitement, la classe \TT{Stage} compose le r\'esultat final du traitement en appelant l'op\'erateur \TT{operator+} de chaque \TT{Node} dans un loup. Le listing~\ref{ImplementationOverridingOperator} montre l'impl\'ementation de l'\TT{operator+} dans l'un d'op\'erateurs finaux de \TT{PpFf} : \TT{Collector}. Dans ce cas, le r\'esultat partiel produit par chaque thread est ajout\'e dans un conteneur STL.

\GT{Je crois qu'il faudrait indiquer/rappeler de quelle fa\c{c}on ces
r\'esultats interm\'ediaires sont combin\'es.}

\IC{J'ai d\'ecrit en quelques mots le processus pour la composition du r\'esultat final.}
