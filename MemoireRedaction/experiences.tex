...
\gt{J'ai chang\'e <<Stock Market>> pour <<StockPrice>> car c'est ce
...
\caption{Les nombres de mots trait\'es par unit\'e de temps de~\PpFf-2.\label{experiencesPpFf2.tab}}\\
\hline
\textbf{Nombre de mots} & \textbf{Temps d'ex\'ecution en ms} & \textbf{Nombres de mots trait\'es par unit\'e de temps}\\
\hline
\endfirsthead
\multicolumn{3}{c}%
...
Une autre remarque int\'eressante qui peut \^etre observ\'ee dans ce diagramme est que le temps d'ex\'ecution augmente de fa\c {c}on lin\'eaire en fonction du nombre de mots trait\'es. En prenant comme exemple le diagramme de \TT{PpFf-2}, le tableau~\ref{experiencesPpFf2.tab} montre les temps d'ex\'ecutions obtenus par \TT{PpFf-2} et les nombres de mots trait\'es par unit\'e de temps. Dans ce tableau on peut observer que pour la plus petite charge de travail, \TT{PpFf} traite 521 mots par ms et 553 pour le plus gros fichier. Cela d\'emontre que \TT{PpFf} est efficace non seulement pour un petit volume de travail, mais aussi pour de grands traitements de donn\'ees.  
...
\IC{J'ai discut\'e un peu du temps d'ex\'ecution}
...
%    	\end{axis}
%	\end{tikzpicture}
%    \caption{Les temps d'ex\'ecution pour l'application \TT{StockPrice}.}
...
\centering
	\begin{tikzpicture}
    	...
    	\end{axis}
	\end{tikzpicture}
    \caption{Les temps d'ex\'ecution pour l'application \TT{StockPrice} sur les deux machines~\M1 et \M2.}
    \label{executionTimesStockPrice.fig}
\end{figure}
...
\GT{Ici, avant de comparer Java/PpFF sur les deux machine, il faut
dire --- parce que ce n'est pas clair --- ce que repr\'esente le
nombre indiqu\'e.  Est-ce que 232 est le temps total d'ex\'ecution?
Pour un grand nombre d'actions? Ou vraiment pour une seule action?  Ou
est-ce le temps {\bf moyen} pour traiter une action, alors que ton
programme en traite un grand nombre. Ce n'est pas clair!  Parce que le
texte ci-bas semble laisser penser que c'est le temps pour traiter une
(1) seule action. Est-ce vraiment le cas?}

\IC{J'ai ajout\'e plus de d\'etails. Je ne sais pas si c'est plus clair.}

Les r\'esultats  pour l'application \TT{StockPrice} sont pr\'esent\'es dans la figure~\ref{executionTimesStockPrice.fig}. Les expériences ont aussi \'et\'e menées sur les machines \M1, \M2. Elles sont repr\'esent\'ees sur l'axe x de la figure~\ref{executionTimesStockPrice.fig}. Chaque programme calcule le prix maximum pour plusieurs actions. Le temps d'ex\'ecution r\'esultant est repr\'esent\'e sur l'axe y de la m\^eme figure~\ref{executionTimesStockPrice.fig}. On remarque que dans les deux cas, \TT{PpFf} obtient de meilleurs temps d'ex\'ecution en comparaison avec \TT{Java}. Sur la machine \M1, \TT{PpFf} est plus de 7 fois plus rapide que \TT{Java}. Tandis que \TT{PpFf} trouve le prix maximum de l'action en seulement 232~ms, Java requiert 1~629~ms pour la m\^eme t\^ache. Un meilleur temps d'ex\'ecution est obtenu par \TT{Java} sur la machine \M2. Avec un temps d'ex\'ecution de 411~ms, \TT{Java} est presque 4 fois plus rapide en comparaison avec le r\'esultat obtenu sur la machine \M1. M\^eme si \TT{Java} se comporte mieux sur cette machine, son ex\'ecution n'est pas meilleure que celle de \TT{PpFf}. Avec 304~ms, \TT{PpFf} est toujours plus rapide que \TT{Java}. 
