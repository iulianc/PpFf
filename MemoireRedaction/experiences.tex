...
\IC{Bonne observation! Je l'ai vu, mais j'ai oublié de le modifier. C'est fait}
...
Dans la section~\ref{descriptionWordCount.sect}, nous avons pr\'esent\'e \TT{WordCount}, une application simple qui compte le nombre d'occurrences des divers mots dans un fichier texte. L'application prend en entr\'ee un fichier texte et produit un conteneur de type \TT{map<string, int>} où la cl\'e repr\'esente un mot dans le fichier et la valeur  type \TT{int}  associ\'ee repr\'esente le nombre d'occurrences du mot dans le fichier. Extraits des programmes utilis\'es pour les exp\'erimentations pour \TT{WordCount} en~\TT{Java}, \TT{C++ Seq}, \TT{PpFf} et \TT{FastFlow} sont pr\'esent\'es dans l'annexe~\ref{appendice-code-experiences.ann}.

\GT{Ci-haut: Si je me souviens bien, les évaluateurs n'avaient pas
apprécié d'avoir le code source en annexe, et non plus en
landscape. Donc, assurément, je ne crois pas approprié, étant donné
les remarques des évaluateurs, de donner ainsi tout le listing. Une
possibilité pourrait --- peut-être --- être de simplement mettre en
appendice des {\bf extraits} du code, pour montrer uniquement les
parties {\bf vraiment significatives}, i.e., uniquement le segment de
code dont le temps d'exécution est mesuré (si nécessaire avec les
classes ou fonctions auxiliaires requises).  Mais assurément que ça ne
vaut pas le coup de tout mettre~: les imports, la lecture des
arguments, le warm-up (pour Java), l'impression du résultat, etc.
%
Et dans ce cas, alors il serait préférable de tout mettre dans une
seule annexe, avec des sous-sections:
cf.~\ref{appendice-code-experiences.ann} pour un squelette possible,
dans lequel j'ai inclus deux éléments, pour voir ce que ça donnerait:
ça me semble assurément mieux ainsi... donc il faudrait compléter.}

\IC{J'ai complété les extraits des programmes}

\subsection{Mesures obtenues et analyse des r\'esultats}

\GT{Bon, j'ai modifié certains scripts et j'ai ajouté une macro
grapheH parce que ça prenait vraiment beaucoup trop de place avec
l'autre façon, et c'était difficile de bien comparer en un seul coup
d'oeil.  Donc, tu vas voir ci-bas que les temps (ou débits, ou accs.)
pour les 3 machines sont présentées sur la même page.}

\IC{C'est mieux. Les graphes peuvent être comparés.}
...
\GT{Encore ci-bas: threads!}

\IC{J'ai remplacé tous les mots thread dans cette section.}

Dans cette section, nous \'evaluons l'application \TT{WordCount} en examinant le temps d'ex\'ecution, le d\'edit et l'acc\'el\'eration sur les trois machines : \M1, \M2 et \M3. Tel que d\'ecrit dans la section~\ref{usedMethodsForBenchmarks.chap}, des exp\'eriences pr\'eliminaires ont \'et\'e effectu\'ees afin de choisir les meilleures versions. Dans le cas de \TT{Java}, la meilleure version choisie est celle avec \emph{warmup} et \emph{JIT}. Elle est indiquée dans chaque graphe avec la notation \TT{Java*}. Dans le cas de \TT{PpFf} et \TT{FastFlow}, les exp\'eriences ont \'et\'e men\'ees en variant les nombres d'instances parall\`eles d'un farm. Deux instances parall\`eles d'un farm ont \'et\'e utilis\'es sur la machine \M1, huit sur la machine \M2 et seulement un sur la machine \M3. Le suffixe entier dans les indicateurs pour \TT{PpFf} et \TT{FastFlow} dans chaque graphe repr\'esente donc ce nombre d'instances parall\`eles d'un farm, par exemple \TT{PpFf-2} utilise deux instances parall\`eles d'un farm. Chaque exp\'erience inclut aussi le programme s\'equentiel, indiqué sur chaque graphe par \TT{Seq}, utilisant  les m\^emes fonctions auxiliaires que \TT{PpFf} et \TT{FastFlow}.
...
\GT{Je crois qu'il faut en dire un peu plus sur les débits, puisque tu
n'en parles pas avant.}

\IC{J'en ai parlé un peu dans le dernier paragraphe de cette section.}
