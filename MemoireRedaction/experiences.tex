...
\GT{Bon, je ne crois pas nécessaire d'inclure ces figures. Mais je les
laisse ici, juste au cas.}
...
L'application choisie pour ces exp\'eriences est l'application \TT{WordCount}, d\'ecrite dans la section~\ref{descriptionWordCount.sect}. Afin de montrer les coûts associés à la gestion des \emph{threads}, trois versions de \TT{WordCount} ont \'et\'e cr\'e\'ees : \TT{WordCountSplitted} avec un pipeline comptant cinq op\'erations, \TT{WordCount} avec quatre opérations, et \TT{WordCountMerged} avec seulement trois op\'erations. On doit pr\'eciser que le r\'esultat obtenu en ex\'ecutant les trois versions de \TT{WordCount} est identique et que l'appel à \TT{parallel()} (voir ci-bas) n'est pas compté dans les opérations. Des extraits du code utilis\'e pour la construction de trois versions sont pr\'esent\'es dans l'annexe~\ref{appendice-code-wordcount-sf.ann}. 
...
La version \TT{WordCount} est cr\'e\'ee \`a partir de la version \TT{WordCountSplitted} en fusionnant les appels aux m\'ethodes \TT{map} et \TT{flatten}, qui sont remplac\'es par un appel à \TT{flatMap}.
...
Nous allons maintenant \'evaluer les trois versions de \TT{WordCount} en examinant les temps d'ex\'ecution sur la machine \M3. Deux s\'eries d'exp\'eriences ont \'et\'e men\'ees en variant les nombres d'instances parall\`eles d'un \emph{farm}. Dans la premi\`ere s\'erie, nous avons utilis\'e deux instances parall\`eles d'un \emph{farm} et dans la deuxi\`eme seulement une instance.  Les r\'esultats sont présentés dans la figure~\ref{WordCount-merged-splitted-temps.fig}. Les trois versions de \TT{WordCount} (\TT{WordCountSplitted}, \TT{WordCount} et \TT{WordCountMerged}) sont identifiées sur les graphes par \ppffs, \ppff\ et \ppffm\ suivi d'un suffixe entier, qui repr\'esente le nombre d'instances parall\`eles d'un \emph{farm}. Par exemple, \ppffs\TT{-2} repr\'esente la version du programme \TT{WordCountSplitted} qui utilise deux instances parall\`eles d'un \emph{farm} (\TT{parallel(2)}).
...
Un effet semblable peut \^etre observ\'e pour la deuxi\`eme s\'erie d'exp\'eriences, avec \TT{parallel(1)}. Dans ce cas, les diff\'erences de temps d'ex\'ecution entre les trois versions de programmes sont beaucoup plus petites. L'utilisation d'une seule instance parall\`ele d'un \emph{farm} entra\^ine l'utilisation d'un seul \emph{thread} suppl\'ementaire entre les versions de programmes. Par exemple la version \TT{WordCount} ex\'ecut\'ee en utilisant quatre threads (4419.6 ms) est plus rapide avec seulement 100 ms que la version \TT{WordCountSplitted} qui utilise cinq threads (4526.6 ms). 

\GT{Je crois qu'il serait bien de souligner que, quand même, les
versions \ppff\TT{-2} et \ppffm\TT{-2} sont quand même plus rapides
que les versions avec \TT{-1}. Peut-être indiquer explicitement (entre
parenthèses) les temps moyens d'exécution.}

\IC{En ajoutant les temps d'exécution, j'ai constaté que pour la série de expériences parallel(1), WordCountMerged est moins performante que les deux autre versions WordCount et WordCountSplitted. J'ai modifié le texte en concéquence.}

Malgr\'e cette faiblesse, le parall\'elisme dans \TT{PpFf} est tr\`es important. Cela peut \^etre constat\'e si on compare les temps moyens d'ex\'ecution pour la m\^eme s\'erie d'exp\'eriences, \TT{parallel(1)}, entre la version \TT{WordCountMerged} et les deux autres versions. En utilisant trois \emph{threads}, la version \TT{WordCountMerged} avec un temps d'ex\'ecution de 4569.6 ms est moins performant que les versions \TT{WordCount} (4 \emph{threads}) et \TT{WordCountSplitted} (5 \emph{threads}). Cet effet est plus \'evident si on compare les temps d'ex\'ecution entre les deux s\'eries des exp\'eriences. Les versions \ppffs\TT{-2} (3730.5 ms), \ppff\TT{-2} (3079.8 ms) et \ppffm\TT{-2} (2479.9 ms) sont plus rapides que les versions \ppffs\TT{-1} (4526.6 ms), \ppff\TT{-1} (4419.6 ms) et \ppffm\TT{-1} (4569.6 ms).
...
Dans ce chapitre, plusieurs exp\'eriences ont \'et\'e présentées dans le but d'évaluer les performances de \TT{PpFf}. Dans la premi\`ere partie de ce chapitre, deux applications utilisant la bibliothèque \TT{PpFf} ont \'et\'e compar\'e avec des programmes Java \'equivalents utilisant les \TT{Streams} introduits en Java 8. Diverses unit\'es de mesures --– temps d'ex\'ecution, d\'ebit et acc\'elération --- ont servi comme r\'ef\'erence pour comparer ces programmes. Les r\'esultats de la plupart des exp\'eriences ont montr\'e que \TT{Java} se comporte mieux que \TT{PpFf}. 
%
D'autres exp\'eriences ont aussi \'et\'e fait pour comparer \TT{PpFf} avec \TT{FastFlow}. Le r\'esultat de ces exp\'eriences a montr\'e que les surco\^uts introduits par \TT{PpFf} sont peu \'elev\'es.


\GT{Tant qu'à être dans un résumé/rappel, tu devrais aussi indiquer que, d'un autre coté, des comparaisons ont aussi été faites avec FastFlow, et ont montré que les surcoûts introduits par Ppff étaient peu élevés!}

\IC{J'ai indiqué la comparaison entre PpFf et FastFlow.}


\GT{Ci-bas: il faut faire attention aux affirmations faites dans une
telle conclusion, surtout avec les deux autres évaluateurs
semble-t-il~:( Donc, il ne m'apparait pas clair en quoi les
expériences de la dernière section montrent que la bibliothèque est
efficace, puisque ces expériences ne comparent ni avec des versions
Java, ni avec des versions séquentielles. J'ai donc supprimé la
dernière phrase et je l'ai remplacé par une phrase qui me semble mieux
décrire ce qui a été démontré par ces expériences.}

\IC{Bonne observation.}

Dans la derni\`ere section de ce chapitre (Sect.~\ref{autres-experiences-wordcount.sect}), d'autres exp\'eriences ont \'et\'e men\'ees pour comparer diff\'erentes versions du programme \TT{WordCount}, versions cr\'e\'ees enti\`erement \`a l'aide de la biblioth\`eque \TT{PpFf}.
%
