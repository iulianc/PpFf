...
\grapheH{WordCount-temps-java-10010-40}
\caption{Temps~: Avant vs.\ Après optimisations.} 
\label{temps-avant-apres.fig}
\end{figure}

\begin{figure}[htbp]
\grapheH{WordCount-debits-java-1001-40}
\grapheH{WordCount-debits-java-10010-40}
\caption{Débits~: Avant vs.\ Après optimisations.} 
\label{debits-avant-apres.fig}
\end{figure}

\begin{figure}[htbp]
\grapheH{WordCount-accs-java-1001-40}
\grapheH{WordCount-accs-java-10010-40}
\caption{Accélérations~: Avant vs.\ Après optimisations.} 
\label{accs-avant-apres.fig}
\end{figure}
...
\grapheH{WordCount-temps-japet-10020-40}
\caption{Temps~: Avant vs.\ Après optimisations.} 
\label{temps-avant-apres.fig}
\end{figure}
...
\grapheH{WordCount-debits-japet-10020-40}
\caption{Débits~: Avant vs.\ Après optimisations.} 
\label{debits-avant-apres.fig}
\end{figure}
...
\grapheH{WordCount-accs-japet-10020-40}
\caption{Accélérations~: Avant vs.\ Après optimisations.} 
\label{accs-avant-apres.fig}
\end{figure}
...
\GT{Attention: ce sont toujours les mêmes expériences, pour temps,
débits et accélérations.  Ce sont simplement des calculs différents,
des résultats différents extraits des temps d'exécution pour ces
expériences.}

\IC{C'est vrai. Vous m'avez déjà expliqué ça dans les remarques précédentes.}
...
En examinant les temps d'ex\'ecution sur les trois machines, on peut constater que \TT{Java} est plus rapide que \TT{PpFf}. On verra dans la section~\ref{limitesppff.sect} pourquoi \TT{Java} est plus performant. Par contre, si on compare \TT{PpFf} et \TT{FastFlow}, les diff\'erences -- tant pour le temp d'ex\'ecution que pour le d\'ebit -– sont n\'egligeables. Cela d\'emontre que les surco\^uts introduits par \TT{PpFf} par rapport \`a \TT{FastFlow} sont quasi inexistants.

\`A partir des m\^emes s\'eries d'exp\'eriences, nous avons aussi calcul\'e le d\'ebit, soit le nombre de transactions trait\'ees par seconde. Tel que montr\'e dans la figure~\ref{StockPrice-debits.fig}, ces d\'ebits ont \'et\'e calcul\'es comme suit : les d\'ebits moyens repr\'esent\'es par les valeurs qui composent la courbe sur le graphe et les d\'ebits minimums et maximums repr\'esent\'es par des petites barres verticales. Un point int\'eressant, qui peut \^etre observ\'e dans les graphes de d\'ebits, est que la dispersion du d\'ebit pour \TT{Java} est plus grande que celle de \TT{PpFf}. Cela d\'emontre que \TT{PpFf} est plus stable que \TT{Java}. 
