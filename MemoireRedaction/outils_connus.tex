...
\IC{Je vais y réflechir. C'est un bon point. Ce sera peut-être une bonne idée de parler aussi de la différence de style entre PpFf et FastFlow. Je crois que l'un d'évaluateurs l'a méntionné dans ses commentaires.}
...
\TT{SkePU}~\citep{skePuReferenceEnLigne} est une biblioth\`eque C ++ con\c {c}ue pour les syst\`emes informatiques parall\`eles h\'et\'erog\`enes. Il propose un ensemble de squelettes algorithmiques pour sp\'ecifier les t\^aches parall\`eles sur des syst\`emes \TT{multi-GPU} utilisant \TT{CUDA} et \TT{OpenCL}.

Pr\'esentement, \TT{SkePU} fournit sept fonctions squelettes \`a usage g\'en\'eral pour traiter les donn\'ees en parall\`ele : \TT{Map}, \TT{Reduce}, \TT{MapReduce}, \TT{MapArray}, \TT{MapOverlap}, \TT{Scan} and \TT{Generate}. Ces fonctions sont impl\'ementées pour diff\'erents types de processeurs et plates-formes, y compris CPU s\'equentiel et multicœurs en utilisant \TT{OpenMP} et \TT{OpenCL} et \TT{CUDA} pour l'ex\'ecution mono et \TT{multi-GPU}.

Dans \TT{SkePU} les donn\'ees sur lesquelles les fonctions op\'eraient sont conserv\'ees dans conteneurs.  Appel\'es conteneurs intelligents, ces conteneurs sont disponibles en tant que vecteurs et matrices avec des \'el\'ements de type g\'en\'erique. Les donn\'ees sont transf\'er\'ees uniquement lorsque cela est n\'ecessaire. Si par exemple, les donn\'ees d'entr\'ee sont utilis\'ees pour un autre calcul, elles ne sont pas copi\'ees \`a nouveau. De plus, les donn\'ees r\'esultantes des calculs interm\'ediaires restent disponibles dans la m\'emoire \TT{GPU} pour des calculs ult\'erieurs. Elles sont transf\'er\'ees dans la m\'emoire h\^ote lorsque cela est n\'ecessaire. Ce m\'ecanisme s'appelle \emph{lazy memory copy}.

\emph{Lazy memory copy} a pour r\^ole de r\'eduire le transfert de donn\'ees redondantes. Le vecteur dans \TT{SkePU} garde une trace des parties qui sont allou\'ees et t\'el\'echarg\'ees sur le \TT{GPU}. Si un calcul est effectu\'e, en changeant le vecteur dans la m\'emoire \TT{GPU}, il n'est pas imm\'ediatement transf\'er\'e dans la m\'emoire h\^ote. La copie n'est effectu\'ee que si l'\'el\'ement est utilis\'e.

\TT{SkePU} donne aux d\'eveloppeurs la possibilit\'e de d\'efinir leurs propres op\'erateurs. Cela est r\'ealis\'e via \emph{user functions}. \emph{User functions} sont des op\'erateurs utilis\'es pour instancier et initialiser une fonctionne squelette. Dans \TT{SkePU} il y deux fa\c {c}ons de d\'efinir un \emph{user fonction} : par une fonction \emph{template} ou par une expression lambda. Une fonction \emph{template} est toujours pr\'ef\'erable au cas o\`u elle pourrait \^etre partag\'ee entre des instances.

Malgr\'e le fait que \TT{SkePU} supporte une large gamme de squelettes algorithmiques en cachant les d\'etails du parall\'elisme, de la communication et de la synchronisation, il reste une biblioth\`eque de bas niveau de traitement de flux de donn\'ees. Par rapport \`a \TT{PpFf}, dans \TT{SkePU} les fonctions sont d\'eclar\'ees de fa\c {c}on imp\'erative et non implicite. \TT{PpFf} propose une mani\`ere \'el\'egante de d\'efinir les fonctions via cha\^inage de m\'ethodes. De plus, \TT{PpFf} offre des m\'ethodes pr\'ed\'efinies pour les op\'erateurs les plus utilis\'es sur le traitement du flux.
