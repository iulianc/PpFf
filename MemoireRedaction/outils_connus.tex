...
\IC{J'ai ajouté quelques mots pour comparer les biblioth\`eques.}

Les deux biblioth\`eques, \TT{RaftLib} et \TT{PpFf} ont plusieurs caract\'eristiques en commun. Elles sont \'ecrites en utilisant les \emph{templates} de \TT{C++} avec une parall\'elisation automatique. Les nœuds dans le flux pour \TT{RaftLib} et \TT{PpFf} s'ex\'ecutent dans des \emph{threads} diff\'erents. Dans ce contexte, \TT{PpFf} ajoute plus de flexibilit\'e en utilisant le parall\'elisme optionnel introduit par les instances parall\`eles d'un \TT{farm}. 

M\^eme si les deux biblioth\`eques ont plusieurs caract\'eristiques en commun, leur conception est diff\'erente. Tandis que \TT{RaftLib} a \'et\'e con\c {c}u pour prendre en charge le traitement des flux par lot et distribu\'e sur des plates-formes h\'et\'erog\`enes, \TT{PpFf} a \'et\'e con\c {c}u pour traiter des donn\'ees en flux incr\'emental, mais aussi des collections en lot. Avec un style fonctionnel, \TT{PpFf} vise la simplicit\'e et l'efficacit\'e en utilisant le cha\^inage de m\'ethodes et les expressions \TT{lambda}.
...
\GT{Ci-bas: pas clair à quoi réfère <<le code existant>>. Du code
séquentiel, comme on ferait avec OpenMp, qu'on rend parallèle avec des
annotations? Ou autre?  En fait, même remarque que ci-haut: ajouter
une phrase qui distingue cette approche de celle de PpFf en termes du
modèle de programmation --- modèle pour PpFf = chainage de méthodes
fonctionnelles, chaque méthode effectuant un traitement spécifique du
flux.}
\IC{<<Le code existant>> doit être n'importe quel code. Plus précisément, c'est une fonction. Le codelet garde une référence vers une fonction et le codelet à son tour c'est une référence à une tâche. J’ai modifié la description et j’ai ajouté une phrase qui distingue cette approche de celle de PpFf.}


\GT{Toujours ci-bas: habituellement, le terme <<unité de calcul>>
réfère à une instance d'une machine physique, i.e., coeur, processeur,
etc.  Donc, dans le cas des codelets, comme il s'agit de code
(logiciel) et non matériel, ce n'est probablement pas le meilleur
terme. Quel est le terme en anglais?}
\IC{Par <<unité de calcul>> je voulais dire qu'un codelet peut fonctionner sur un CPU ou GPU. Le terme en anglais est <<computational kernel>>. J'ai modifé la description.}

Un programme \TT{StarPU} est \'ecrit sous forme de graphique de t\^aches, chaque t\^ache travaillant sur un ensemble de donn\'ees. Le programmeur doit sp\'ecifier pour chaque t\^ache les donn\'ees d'entr\'ee et de sortie, cr\'eant ainsi des d\'ependances entre les t\^aches. De cette fa\c {c}on, une application \TT{StarPU} g\'en\`ere un graphique des t\^aches au moment de l'ex\'ecution. Les t\^aches sont asynchrones. Autrement dit, soumettre une t\^ache \`a \TT{StarPU} est une op\'eration non bloquante. Comme tout autre syst\`eme, \TT{StarPU} introduit des co\^uts pour g\'erer les t\^aches. Ces co\^uts peuvent \^etre importants si la quantit\'e de travail \`a effectuer n'est pas assez grande.

La partie de calcul de la t\^ache est appel\'ee \emph{codelet}. En effet, dans  \TT{StarPU}, l'ex\'ecution d'une t\^ache consiste \`a appliquer un \emph{codelet} sur une structure de donn\'ees. Un \emph{codelet} peut ensuite \^etre impl\'ement\'e sur des architectures h\'et\'erog\`enes, telles que \TT{CUDA} ou \TT{OpenCL}.

Dans \TT{PpFf}, ce processus de cr\'eation de la cha\^ine de traitement dans le flux est beaucoup simplifi\'e par l'utilisation du chainage de m\'ethodes fonctionnelles. Chaque m\'ethode effectu\'e un traitement sp\'ecifique dans le flux. De plus, dans \TT{PpFf} le \emph{codelet} \TT{StarPU} peut \^etre d\'efini lors de la d\'eclaration du flux de traitement \`a l'aide d'expressions \emph{lambda}. Avec moins de code, les expressions \emph{lambda} facilitent la compr\'ehension de la partie de traitement du flux.
...
\GT{Ci-bas: je ne comprends pas ce que tu veux dire par <<déclarées de
façon [\ldots] non implicite>>?}
\IC{Je voulais dire implicite via chainage de méthodes. J'ai effacé le mot implicite}
