...
\GT{Ci-bas~: Si tu dis que Spart est pour r\'esoudre certaines
...
\TT{Apache Spark}~\citep{apachSpark} est un flux de traitement de donn\'ees rapide dot\'e avec un expressif \TT{API} qui permet aux d\'eveloppeurs d'ex\'ecuter efficacement plusieurs op\'erations sur un set de donn\'ees qui n\'ecessite un acc\`es it\'eratif rapide. \TT{Spark} est un outil de traitement par lots qui a aussi des capacit\'es de traitement de flux de donn\'ees. Il se concentre principalement sur l'acc\'el\'eration du traitement des donn\'ees en gardant en m\'emoire les donn\'ees de travail, ce qui permet notamment l'acc\'el\'eration du traitement des algorithmes it\'eratifs.
...
Afin d'implémenter le traitement de donn\'ees, Spark utilise le mod\`ele appel\'e \TT{Resilient Distributed Datasets (RDD)} \citep{Salloum2016} qui g\`ere efficacement la m\'emoire. De plus, il impl\'emente le mod\`ele \TT{Discretized Streams} \citep{zaharia2013discretized} pour le traitement de flux de donn\'ees.


\subsection{Resilient Distributed Dataset (RDD)}

Un~; \TT{RDD} est une abstraction de m\'emoire qui \'evite la r\'eplication de donn\'ees et minimise les acc\`es au disque. L'utilisation de  RDD permet aux applications de mettre en cache des donn\'ees \`a travers diff\'erentes \'etapes de traitement, ce qui acc\'el\`ere consid\'erablement la r\'eutilisation pour les donn\'ees futures. De plus, les RDD m\'emorisent les op\'erations utilis\'ees pour les construire et utilisent un m\'ecanisme de \emph{checkponint}. Lorsqu'une panne survient, les RDD requis peuvent \^etre reconstruits, mais sans n\'ecessairement devoir tout refaire les calculs.
