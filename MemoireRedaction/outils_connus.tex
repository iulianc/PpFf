...
Afin de r\'eutiliser le code existant, \TT{StarPU} introduit la notion de \emph{codelet}. Un \emph{codelet} d\'ecrit une unit\'e de calcul. Plut\^ot que de r\'e\'ecrire tout le code, les programmeurs peuvent encapsuler des fonctions existant dans des \emph{codelets}. En effet, dans  \TT{StarPU}, l'ex\'ecution d'une t\^ache consiste \`a appliquer un \emph{codelet} sur une structure de donn\'ees. Un \emph{codelet} peut ensuite \^etre impl\'ement\'e sur des architectures h\'et\'erog\`enes, telles que \TT{CUDA} ou \TT{OpenCL}.  
...
Dans \TT{SkePU}, les donn\'ees sur lesquelles les fonctions opèrent sont conserv\'ees dans conteneurs dits <<intelligents>>. Ces conteneurs sont disponibles sous forme de vecteurs et matrices avec des \'el\'ements de type g\'en\'erique. Les donn\'ees sont transf\'er\'ees uniquement lorsque cela est n\'ecessaire. Si, par exemple, les donn\'ees d'entr\'ee sont utilis\'ees pour un autre calcul, elles ne sont pas copi\'ees \`a nouveau. De plus, les donn\'ees r\'esultant de calculs interm\'ediaires restent disponibles dans la m\'emoire \TT{GPU} pour des calculs ult\'erieurs. Elles sont transf\'er\'ees dans la m\'emoire h\^ote lorsque cela est n\'ecessaire. Ce m\'ecanisme s'appelle \emph{lazy memory copy}.


\GT{Ci-bas (à plusieurs endroits): Le vecteur\ldots\ ==> Le conteneur?}

Le \emph{lazy memory copy} a pour r\^ole de r\'eduire le transfert de donn\'ees redondantes. Le vecteur dans \TT{SkePU} garde une trace des parties qui sont allou\'ees et t\'el\'echarg\'ees sur le \TT{GPU}. Si un calcul est effectu\'e, en changeant le vecteur dans la m\'emoire \TT{GPU}, il n'est pas imm\'ediatement transf\'er\'e dans la m\'emoire h\^ote. La copie n'est effectu\'ee que si l'\'el\'ement est utilis\'e.

\TT{SkePU} donne aux d\'eveloppeurs la possibilit\'e de d\'efinir leurs propres op\'erateurs. Cela est r\'ealis\'e via des \emph{user functions}, lesquelles sont sont des op\'erateurs utilis\'es pour instancier et initialiser une fonction squelette. Il y a deux fa\c {c}ons de d\'efinir une \emph{user fonction} : par une fonction \emph{template} ou par une expression lambda. Une fonction \emph{template} est toujours pr\'ef\'erable car elle pourrait ensuite \^etre partag\'ee entre des instances.


\GT{Ci-bas: je ne comprends pas ce que tu veux dire par <<déclarées de
façon [\ldots] non implicite>>?}

Malgr\'e le fait que \TT{SkePU} supporte une large gamme de squelettes algorithmiques en cachant les d\'etails du parall\'elisme, de la communication et de la synchronisation, cela reste une biblioth\`eque de bas niveau de traitement de flux de donn\'ees. Par rapport \`a \TT{PpFf}, dans \TT{SkePU} les fonctions sont d\'eclar\'ees de fa\c {c}on imp\'erative et non implicite. \TT{PpFf} propose une mani\`ere plus simple de d\'efinir les traitement via le cha\^inage de m\'ethodes. De plus, \TT{PpFf} offre de nombreuses m\'ethodes pr\'ed\'efinies pour les op\'erations les plus utilis\'ees de traitement de flux.
