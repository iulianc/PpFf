...
\GT{À moins que tu en dises beaucoup plus, pas besoin il me semble
d'avoir des sous-sections.}
...
%
\GT{Ci-bas: tu veux dire <<échanger>> plutôt que <<changer>>?}
%
...
%
\GT{Il manque quelque chose ici: type de base? type primitif?}
...
\GT{Après lecture, pas clair pour moi la différence entre allocate et
allocate\_s. Pas certain non plus que ce soit nécessaire de traiter de
cette différence. Peut-être simplement omettre ce paragraphe?}

Dans \TT{RaftLib}, deux m\'ethodes de type \emph{zero copy} sont d\'efinies : \TT{allocate()} et \TT{allocate\_s()}. La premi\`ere m\'ethode, \TT{allocate()}, retourne une r\'ef\'erence \`a la fin de la file \emph{FIFO}. Une fois que quelque chose est \'ecrit, un appel \`a la m\'ethode \TT{send()} doit \^etre effectu\'e pour envoyer les donn\'ees dans la file. Contrairement \`a \TT{allocate()}, la deuxi\`eme m\'ethode, \TT{allocate\_s()} retourne un objet au lieu d'une r\'ef\'erence. Dans ce cas, l'utilisateur doit d\'er\'ef\'erencer l'objet pour acc\'eder \`a la m\'emoire.

\GT{Idem pour cet autre paragraphe. Je ne sais pas si ce sont des
détails utiles à connaitre?}
...
Les m\'ethodes avec copie d\'efinies dans \TT{RaflLib} sont \TT{push()} et \TT{insert()}. La m\'ethode \TT{push()} ajoute dans la file une copie de chaque objet cr\'e\'e \`a l'aide de son constructeur. Afin de transf\'erer un conteneur \TT{C++}, la m\'ethode \TT{insert()} peut \^etre utilis\'ee. Elle utilise les deux param\`etres de type it\'erateur, l'un qui pointe vers le d\'ebut du conteneur el l'autre vers la fin. 
