...
\chapter{APIs de traitement de flux de donn\'ees~: \TT{Spark}, \TT{Java} et \TT{FastFlow}}
\label{outils_connus}
\label{outils_connus.chap}

\gt{J'ai chang\'e le titre car  ce ne sont pas vraiment des \underline{langages}!}
...
\GT{Je r\'ealise qu'il manque quelque chose: avant de dire que les
flux de donn\'ees sont utilis\'es \`a plusieurs endroits et dans
plusieurs domaines, il faut commencer par expliquer un peu plus c'est
quoi un flux de donn\'ees!  Par exemple, voir:
\url{https://en.wikipedia.org/wiki/Streaming_data}
%
OU
%
\url{https://fr.wikipedia.org/wiki/Flux_(informatique)}
OU
\url{https://en.wikipedia.org/wiki/Flow-based_programming}
}


\GT{Il me semble qu'il faudrait aussi mentionner le fait qu'on peut
traiter une collection de donn\'ees comme un flux, m\^eme si cette
collection n'est pas a priori un <<vrai>> flux --- pas une suite
infinie de valeurs.  Donc, ma suggestion serait d'expliquer tout
d'abord l'id\'ee de flux de donn\'ees au sens des \emph{streams},
i.e., des s\'eries potentiellement infinies de donn\'ees --- qui
doivent n\'ecessairement \^etre trait\'es de fa\c{c}on incr\'ementale
--- trait\'es par des processus interconnect\'es.  Ensuite,
d'expliquer que l'id\'ee de tels traitements peut aussi \^etre
utilis\'ee pour des collections \textbf{finies} de donn\'ees.  Est-ce
clair? Si non, on peut en jaser \`a un moment par Skype...}

\IC{J'ai modifi\'e un peu les explicationsh.}


Un \emph{flux de donn\'ees} est une s\'erie potentiellement infinie de donn\'ees g\'en\'er\'ees par diff\'erentes sources qui doivent n\'ecessairement \^etre trait\'ees de fa\c {c}on incr\'ementale par des processus interconnect\'es. Le traitement des donn\'ees dans un flux s'effectue \`a l'aide d'une s\'equence d'op\'erations pr\'ed\'efinie. Les op\'erations courantes de traitement de flux des donn\'ees comprennent le tri, le calcul, le filtrage et la transformation des donn\'ees. Lorsque ces op\'erations sont ex\'ecut\'ees avec diff\'erents \TT{threads}, on parle du parall\'elisme de flux de donn\'ees. Le traitement complet est atteint pour les donn\'ees en les faisant migrer d'une op\'eration \`a une autre dans l'ordre dans lequel elles apparaissent dans le traitement.

Un tel traitement peut aussi \^etre utilis\'e pour des collections finies de donn\'ees. Connu comme le mod\`ele de traitement par lots (batch en anglais), ce traitement n\'ecessite que les donn\'ees soient collect\'ees au fil du temps avant d'\^etre trait\'ees.
...
\ic{J'ai ajout\'e le paragraphe ci-dessous.}

\gt{Je ne comprends pas trop la premi\`ere phrase~:( Peut-\^etre
revoir les URLs ci-haut!}
...
\ic{J'ai effac\'e le paragraphe redondant}
...
\item \TT{textFile}, qui lire les lignes \`a partir d'un fichier. Le fichier est rep\'er\'e par le param\`etre fourni en argument \`a \TT{textFile}.
...
\item \TT{mapToPair}, qui cr\'ee une paire (de type Tuple2) avec une cl\'e repr\'esent\'ee par le mot et une valeur associ\'ee \'egale \`a 1.

\item \TT{reduceByKey}, qui regroupe les mots similaires ensemble et compte le nombre d'occurrences de chaque mot.


\GT{Ci-haut: il faut compl\'eter les explications~;)} 
\IC{J'ai compl\'et\'e les explications.}
\end{itemize}
...
\ic{J'ai remplac\'e <<Files.lines>> par <<lignes>>}
