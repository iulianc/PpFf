...
Le paradigme de programmation parall\`ele par flux de donn\'ees offre une approche prometteuse pour la programmation de syst\`emes multicœurs. Les flux de donn\'ees sont utilis\'es dans plusieurs domaines, tels que le traitement d'images, sur le r\'eseau, pour la Bourse, le trafic routier, etc.


\GT{Ci-haut: je ne comprends pas ce que tu veux dire par <<autres domaines de \textbf{traitement des m\'edias}>>!?}

\IC{J'ai reformul\'e la phrase}.
...
language=java,
gobble=4,
caption={[Comparaison entre une it\'eration externe et interne.]Comparaison entre une it\'eration externe et interne~: Dans le cas de l'it\'eration externe, le d\'eveloppeur d\'efinit la boucle pour traverser les \'el\'ements de la collection.  La fonction appliqu\'ee \`a chaque \'el\'ement convertit la cha\^ine en lettres majuscules. La m\^eme fonction est appliqu\'ee dans le cas de l'it\'eration interne. Cette fois, le syst\`eme contr\^ole le processus d'it\'eration.
\GT{Il faudrait donner quelques explications additionnelles, pour souligner l'aspect imp\'eratif (externe) vs. fonctionnel (interne). Et aussi dire bri\`evement ce que cela fait!} \IC{J\'ai ajout\'e plus des explications.}},
frame=single,
float]
    List<String> myList =
...
float]
  ...
  List<Map.Entry<String,Integer>> wordsCount = lines.stream()
    .parallel()
...
\IC{J'ai ajout\'e un petit exemple d'ex\'ecution.}
